...
# — INTERACTIVE variant with control-file polling for autotune.
#
# Objective: min_{ε_geom, s_{1:N}} E_x[-ln det J_N(x, s_{1:N}, c(ε_geom))]
#
# Differences from train_dopt.jl:
#   1. β starts at 1.0 (true greyscale — tanh(1·(x-0.5)) is nearly linear),
#      so the continuous design space is explored before binarization.
#   2. Passive β schedule is long (8×200 iters per doubling → β 1→256);
#      the autotune cron agent is expected to drive β via control.toml.
#   3. Polls checkpoints_dopt/control.toml each iter for overrides to
#      {beta, beta_max, freeze_beta, clear_schedule, lr_geom, lr_s,
#       save_every, stop}.
#   4. n_iters extended to 2000 to accommodate the full warmup.
#
# Usage:  julia -p 20 train_dopt_interactive.jl
...
filter_radius  = 5.0
β_proj_init    = 1.0      # greyscale-first (tanh(1·(x-0.5)) ≈ linear)
β_proj_max     = 256.0
β_proj_schedule = [200, 200, 200, 200, 200, 200, 200, 200]  # 8 doublings → β up to 256
...
# Interactive control state (live-tunable via control.toml)
...
for t in 1:n_iters
    global β_proj, ε_geom, m_geom, v_geom, s_list, m_s, v_s,
           lr_geom, lr_s, save_every, β_proj_max
    t_start = time()
    ...
    if isfile(control_path)
        ...
        lr_geom    = Float64(ctrl_state[:lr_geom])
        lr_s       = Float64(ctrl_state[:lr_s])
    ...
    # β continuation: double β_proj at scheduled milestones (unless frozen)
    if !isempty(β_milestones) && t == β_milestones[1] && β_proj < β_proj_max && !(ctrl_state[:freeze_beta] === true)
        β_proj = min(2 * β_proj, β_proj_max)
        popfirst!(β_milestones)
        ctrl_state[:beta] = β_proj
        println("  [optim] β_proj → $β_proj"); flush(stdout)
    elseif !isempty(β_milestones) && t == β_milestones[1] && ctrl_state[:freeze_beta] === true
        popfirst!(β_milestones)
