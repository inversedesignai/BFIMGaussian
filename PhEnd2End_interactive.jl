...
# ──────────────────────────────────────────────────────────────────────────────
# PhEnd2End_interactive.jl
#
# Same as PhEnd2End.jl except that train_adam! polls a TOML "control file"
# at the start of every iteration, allowing live tuning of β_proj, the β
# schedule, lr, αr, resample cadence, save cadence, and graceful stop.
#
# Usage: drop a TOML file at $save_dir/control.toml with any of these keys:
#
#   beta           = 16.0      # override β_proj now
#   beta_max       = 256.0     # raise/lower the cap on automatic doubling
#   freeze_beta    = true      # stop automatic β doubling at milestones
#   clear_schedule = true      # discard all remaining β milestones
#   lr             = 5e-4      # Adam learning rate
#   alpha_r        = 0.01      # regularization on sensor params
#   resample_every = 1         # 0 disables resampling
#   save_every     = 5         # checkpoint cadence
#   stop           = true      # graceful exit after current iter
#
# The file is consumed on first read (renamed to control.toml.applied.<ts>)
# so each edit fires exactly once. Only keys present in the file are applied.
# ──────────────────────────────────────────────────────────────────────────────
...
# ── Interactive control file (TOML overrides applied between iterations) ────

const _CONTROL_KEYS = (:beta, :beta_max, :freeze_beta, :clear_schedule,
                       :lr, :alpha_r, :resample_every, :save_every, :stop)

"""
    apply_control!(path, state) -> Bool

Read a TOML file at `path`, merge recognized keys into `state::Dict{Symbol,Any}`,
and rename the file to `<path>.applied.<timestamp>` so each edit fires once.
Returns `true` if any value changed, `false` otherwise.
"""
function apply_control!(path::String, state::Dict{Symbol,Any})
    ...
    try
        ...
        for (k, v) in cfg
            sym = Symbol(k)
            if sym in _CONTROL_KEYS
                if !haskey(state, sym) || state[sym] != v
                    old = get(state, sym, nothing)
                    println("  [control] $sym: $old → $v"); flush(stdout)
                    state[sym] = v
                    changed = true
                end
            else
                println("  [control] WARNING: unknown key \"$k\" ignored"); flush(stdout)
            end
        end
    catch e
        println("  [control] ERROR parsing $path: $e"); flush(stdout)
    end
    ts = Dates.format(now(), "yyyymmdd_HHMMSS")
    applied = "$path.applied.$ts"
    try
        mv(path, applied; force=true)
        println("  [control] consumed → $applied"); flush(stdout)
    catch e
        println("  [control] WARNING: could not rename control file: $e"); flush(stdout)
    end
    return changed
end
...
function train_adam!(ε_geom, n_geom, ε_base, ω_array, Ls, Bs, grid_info,
                     ...
                     filter_radius=0.0,
                     β_proj_init=1.0, β_proj_max=256.0,
                     β_proj_schedule=[2000, 1000, 500, 100, 50],
                     control_path=joinpath(save_dir, "control.toml"))
    ...
    # β doubles once per schedule entry.  E.g. schedule=[2000,1000,500,100,50]
    # means: run 2000 iters at β=1, then double → β=2; run 1000 more, double → β=4;
    # run 500 more → β=8; etc.  If β_max is not yet reached when the schedule is
    # exhausted, the last interval repeats.
    ...
    if use_proj
        ...
        println("  [optim] β schedule: double at iters $β_milestones (β: $(β_proj_init) → $(min(β_val, β_proj_max)))"); flush(stdout)
    else
        β_milestones = Int[]
    end
    ...
    # Interactive control state (live-tunable via $control_path)
    ctrl_state = Dict{Symbol,Any}(
        :beta           => use_proj ? β_proj : 0.0,
        :beta_max       => β_proj_max,
        :freeze_beta    => false,
    ...
    println("  [optim] Interactive control file: $control_path"); flush(stdout)

    for t in 1:n_iters
        t_start = time()

        # Interactive control: apply any pending overrides at iter boundary
        if isfile(control_path)
            apply_control!(control_path, ctrl_state)
            lr             = Float64(ctrl_state[:lr])
            αr             = Float64(ctrl_state[:alpha_r])
            resample_every = Int(ctrl_state[:resample_every])
            save_every     = Int(ctrl_state[:save_every])
            β_proj_max     = Float64(ctrl_state[:beta_max])
            if use_proj
                β_proj = Float64(ctrl_state[:beta])
            end
            if ctrl_state[:clear_schedule] === true
                empty!(β_milestones)
                ctrl_state[:clear_schedule] = false
                println("  [optim] β schedule cleared"); flush(stdout)
            end
            if ctrl_state[:stop] === true
                println("  [optim] Stop requested via control file. Exiting at iter $t."); flush(stdout)
                break
            end
        end

        # β continuation: double β_proj at scheduled milestones
        if use_proj && !isempty(β_milestones) && t == β_milestones[1] && β_proj < β_proj_max && !(ctrl_state[:freeze_beta] === true)
            β_proj = min(2 * β_proj, β_proj_max)
            popfirst!(β_milestones)
            ctrl_state[:beta] = β_proj
            println("  [optim] β_proj → $β_proj"); flush(stdout)
        elseif use_proj && !isempty(β_milestones) && t == β_milestones[1] && ctrl_state[:freeze_beta] === true
            popfirst!(β_milestones)
            println("  [optim] β milestone at iter $t skipped (freeze_beta=true)"); flush(stdout)
        end
...
const _MODE = lowercase(get(ENV, "BFIM_MODE", "adam"))
    ...
    # Initial geometry: load from checkpoint if available, else random
_init_ckpt = get(ENV, "BFIM_INIT", "")
_init_β    = nothing   # if checkpoint carries β_proj, resume from it
if !isempty(_init_ckpt) && isfile(_init_ckpt)
    _init_data = deserialize(_init_ckpt)
    ε_geom_init = _init_data.ε_geom
    if hasproperty(_init_data, :β_proj) && _init_data.β_proj !== nothing
        _init_β = Float64(_init_data.β_proj)
    end
    # Allow env-var override (useful for manually overriding the resume point)
    _env_β = get(ENV, "BFIM_INIT_BETA", "")
    if !isempty(_env_β)
        _init_β = parse(Float64, _env_β)
    end
    _β_suffix = _init_β === nothing ? "" : ", β_proj=$_init_β"
    println("[init] Loaded ε_geom from $(_init_ckpt) (step=$(_init_data.step), loss=$(round(_init_data.loss, sigdigits=4))$_β_suffix)"); flush(stdout)
else
    ε_geom_init = rand(MersenneTwister(1234), Ny_d, Nx_d)
    if !isempty(_init_ckpt)
        println("[init] WARNING: checkpoint $(_init_ckpt) not found, using random init"); flush(stdout)
    end
end
...
if _MODE == "adam"
    ...
    β_init = _init_β === nothing ? 1.0 : _init_β
    ε_geom_opt, losses = train_adam!(
        ...
        filter_radius=5.0, β_proj_init=β_init, β_proj_max=256.0,
        β_proj_schedule=[500, 300, 200, 100, 50])
...
if _run_test("ekf_perf")
    let n_mc = 50, N_eval = max(N_steps, 10)
        ...
        println("═══ EKF vs MLE Performance Evaluation ═══"); flush(stdout)
        ...
        for ep in 1:n_mc
            ...
            # Accumulators for MLE: all measurements and sensor params so far
            ...
            for k in 1:N_eval
                ...
                # ── MLE: argmin_x Σ_j ‖y_j − f(x, s_j, c)‖² / σ² ──────────
                push!(ys_so_far, yk)
                push!(ss_so_far, sk)
                ...
                mle_obj = x -> begin
                    ...
                    for j in eachindex(ys_k)
                        pred = mf_check.f(x, ss_k[j], c_nom)
        ...
        # ── Report ────────────────────────────────────────────────────────────
        println("\n  Step | EKF rel_err mean±std   | MLE rel_err mean±std   | tr(Σ) mean           | Mahal² | BFIM tr"); flush(stdout)
        ...
        println("    EKF final rel_err = $(round(final_ekf, sigdigits=4))  ($(round(final_ekf/prior_err, sigdigits=3))× prior)"); flush(stdout)
