...
function _lattice_freq_core(k, Δn, n_lat, a_in, S_arr, dSdn_arr, d2Sdn2_arr,
    ...
    A      = I(n_int) - S_cc * P
    # Use inv(A) * b instead of A \ b: Zygote can't differentiate through
    # LAPACK's zgetrs/zgesv (complex dense \), but inv(A) and * have pure-Julia
    # rrules.  The lattice system is small (n_int ≤ ~8), so inv is fine.
    A_inv  = inv(A)
    a_int  = P * (A_inv * (S_ce * a_in))
    a_out  = S_ee * a_in + S_ec * a_int

    da_int = da_in !== nothing ? P * (A_inv * (S_ce * da_in)) : nothing
    da_out = da_in !== nothing ? S_ee * da_in + S_ec * da_int : nothing

    return (; S_b, dS_b, S_ec, A_inv, P, a_in, a_int, a_out,
              da_in, da_int, da_out)
end
    ...
    Js = map(eachindex(S_arr)) do k
        ...
        J_k = map(scat_groups) do sg
            ...
            δa_out = sg.E_ext * (sg.F_ext' * δb) +
                     fc.S_ec * (fc.P * (fc.A_inv * (sg.E_int * (sg.F_int' * δb))))
    ...
    results = map(eachindex(S_arr)) do k
        ...
        cols = map(scat_groups) do sg
            ...
            dδa_out = sg.E_ext * (sg.F_ext' * dδb) +
                      fc.S_ec * (fc.P * (fc.A_inv * (sg.E_int * (sg.F_int' * dδb))))
