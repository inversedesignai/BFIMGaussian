#=
bayesopt_pcrb_5d.jl — 5-D Bayesian optimization of PCRB log_J_P with the
drive frequency ω_d promoted to a FREE 5th parameter.

Schedule fixed to (10, 2)^4 = (320 ns, n=10)^4 (verified optimal across
all our PCRB experiments; the 4-D BayesOpt with per-eval enumeration also
chose this schedule on every probe). Schedule re-verification post-hoc.

Output: results/bayesopt_pcrb_5d/result.jls
...
println("bayesopt_pcrb_5d.jl"); flush(stdout)
...
const TWO_PI = 2π
# Physically-realistic ω_d range: matches the qubit-frequency box.
...
function objective(z)
    ...
    n = length(HIST.logJP)
    @printf("[pcrb5d %3d] log_JP=%+.6f  c=(%.4f, %.4f, %.4f MHz, %.4f)  ω_d/(2π)=%.4f GHz  %.1fs\n",
...
println("\nLaunching 5-D PCRB BOpt with $N_EVAL evals ($N_INIT random init)..."); flush(stdout)
t_run0 = time()
result = boptimize!(opt)
elapsed = time() - t_run0
println("\n5-D PCRB bopt total: $(round(elapsed/60; digits=2)) min"); flush(stdout)

ibest = argmax(HIST.logJP)
v_best = HIST.c_vec[ibest]
ω_best = HIST.omega_d[ibest]
logJP_best = HIST.logJP[ibest]
@printf("\n5-D PCRB BOpt best log_JP = %.6f at eval %d\n", logJP_best, ibest)
...
# Compare with the omega_d_fn(c) value at this c
...
outdir = joinpath(@__DIR__, "results", "bayesopt_pcrb_5d")
...
open(joinpath(outdir, "result.jls"), "w") do io
    serialize(io, (; hist=HIST, ibest, logJP_best, v_best, omega_d_best=ω_best,
                     ...
                     LO5, HI5, optimizer=:BayesOpt_EI_ARDMatern_5D,
                     timestamp=now()))
end
