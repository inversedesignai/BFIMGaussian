...
\title{Beyond the Information Bound:\\
       End-to-End Co-Design of Sensor Hardware and Adaptive Measurement Policy}
...
\begin{abstract}
End-to-end optical inverse design co-optimizes continuous physical hardware with a \emph{one-step} learned reconstruction, with a growing literature explicitly adopting mutual-information or Fisher-information rewards as the training objective~\cite{kellman2019physics, muthumbi2019learned, bostan2018deep, kellman2020datadriven}; adaptive-sensing POMDP theory optimizes a \emph{multi-step adaptive policy} on fixed hardware.  Neither addresses what a real reconfigurable sensor is: a physical geometry whose operator reacts to data as it arrives.  We close this gap.  Using the pointwise expected information-gain functional $\Phi(\bx, \bs, \bc)$ as a family of compatible scalar rewards, we formulate the joint optimization of continuous hardware $\bc$ and Bellman-optimal adaptive policy $\pi$ end-to-end.  An envelope-theorem identity removes the policy Jacobian from the geometry gradient, making the outer loop tractable even when the inner Bellman solver is approximate.  A principled relaxation hierarchy, exact dynamic programming, deep reinforcement learning, certainty-equivalent control, myopic Bayesian $D$-optimal design, and extended-Kalman-filter Gaussian-trace surrogates, renders the formulation portable from 16-state discrete POMDPs to $10^{5}$-pixel photonic topologies.  Three case studies demonstrate the framework at three scales and under three different choices of $\Phi$: (i) a radar beam-search POMDP solved by exact enumeration where the classical information-bound-guided geometry loses a factor of $2.8\times$ in attainable adaptive value to the joint-DP optimum, (ii) a superconducting-qubit flux sensor in which joint dynamic-programming co-design achieves a statistically significant $13.4\%$ reduction in deployed mean-squared error ($z = +11.5\sigma$) over the joint Bayesian Cram\'er--Rao (PCRB) baseline, and (iii) a photonic metasensor with 90{,}000 free design pixels co-optimized via the Bayesian Fisher-information-matrix surrogate.  We show that the central operational claim, joint $(\bc, \pi)$ optimization strictly outperforms the decoupled classical blueprint, holds provided three engineering preconditions are met: a sufficiently rich action grid, a scalar reward aligned with the deployment metric, and a physically realistic hardware box.  We advance the Expected Value of Perfect Information as a useful diagnostic pre-screen, not a go/no-go gate.
\end{abstract}
...
Sensors are a concatenation of a physical front-end (a lens, an antenna aperture, a superconducting circuit, a diffractive metasurface) and a measurement protocol that interrogates the front-end and reconstructs a quantity of interest $\bx$ from noisy outputs $\by$.  Two largely separate research communities have each co-designed \emph{half} of this pipeline.  On the hardware side, the last decade of computational inverse design has established that the physical geometry $\bc$ of an optical element can be optimized end-to-end with a downstream reconstruction, typically a fixed, feed-forward neural network~\cite{sitzmann2018e2e, chang2019diffractive, metzler2020deep, tseng2021metalens, kellman2019microscopy, heide2014flex, antipa2018diffusercam, bhargava2024topology}.  Inverse-designed photonics and metaoptics now routinely reaches the electromagnetic performance bounds of the underlying scattering problem~\cite{lin2019overlapping, lin2018topology, lin2021e2e, christiansen2021fullwave, molesky2018inverse, jensen2011topology, fan2020freeform, wang2018reconfigurable}.  On the policy side, the adaptive-sensing, sensor-management, and sequential-Bayesian-experimental-design communities have established a mature toolkit for constructing \emph{multi-step} measurement policies that adapt to incoming data, but they invariably assume the physical hardware is fixed~\cite{kaelbling1998pomdp, smallwood1973sondik, sondik1971optimal, kreucher2005kastella, hero2011survey, castanon2008stochastic, chen2015adaptive, krause2005near, lindley1956measure, huan2016sequential, shen2023bayesian, alexanderian2021optimal, chaloner1995bayesian}.

These two bodies of work address disjoint halves of the co-design problem.  End-to-end optics is \emph{non-adaptive} in time: the learned reconstruction takes a single-shot observation and returns an estimate; there is no concept of ``what should I measure next given what I have already seen?''  Adaptive sensing, conversely, treats the hardware as immutable.  The missing piece is the joint optimization of \emph{continuous} physical hardware with a \emph{multi-step Bellman-optimal} adaptive measurement policy.  Existing near-relatives, cognitive radar with MIMO waveform adaptation~\cite{bell2015cognitive, haykin2006cognitive, greco2018cognitive, bouhou2025cognitive}, quantum metrology with Kitaev-style phase estimation on fixed circuits~\cite{giovannetti2006quantum, braunstein1994sld, paris2009quantum, caves1981quantum, danilin2024scqubit, kitaev1995qpe, braverman2019slowing}, programmable metasurface imaging with reconstruction-only adaptation~\cite{yurduseven2017printed, imani2020review}, and variational quantum sensing with trainable but fixed ansatze~\cite{pirandola2018advances, degen2017sensing}, each either decouple hardware from policy or approximate the inner loop with a fixed-architecture neural ansatz that lacks a physical-geometry interpretation.

Closing this gap requires a formulation that (a) is cleanly defined across discrete and continuous state spaces, (b) admits a tractable outer gradient with respect to the physical geometry, and (c) provides a hierarchy of relaxations that scales from exact dynamic programming on small POMDPs to high-dimensional continuous hardware optimization.  We supply all three.

\subsection{Contributions}

\begin{enumerate}[leftmargin=2em]
\item \textbf{Unified formulation.}  We identify a \emph{family} of pointwise scalar rewards $\Phi(\bx, \bs, \bc)$ that fit the same joint-$(\bc, \pi)$ Bellman machinery.  Three canonical members are (i) the mutual-information-aligned $\Phi_{\text{MI}}$ following Lindley~\cite{lindley1956measure}, (ii) the Bayesian mean-squared-error aligned $\Phi_{\text{Var}}$ tied to the deployment metric by the tower law of variance, and (iii) the Fisher-information aligned $\Phi_{\log J}$ following Cram\'er--Rao~\cite{cramer1946mathematical, rao1945information}.  Each reward picks out a different operating point; choosing $\Phi$ is a modeling commitment, not a mathematical necessity.
...
\item \textbf{Quantitative findings, from two complementary angles.}  On a radar beam-search benchmark solved by exact enumeration and exact backward induction, the argmax of the classical information-bound $\E[\mathrm{IG}](\bc)$ loses a factor of $2.8\times$ in attainable adaptive value $V_{\text{adaptive}}$ to the joint-DP optimum.  On a continuous-state superconducting-qubit flux sensor, joint-DP co-design with $\Phi_{\text{Var}}$ achieves a statistically significant $13.4\%$ reduction in deployed Bayesian mean-squared error ($z = +11.5\sigma$) over the joint Bayesian Cram\'er--Rao fixed-schedule baseline~\cite{vantrees2007bayesian, tichavsky1998posterior}.  The two findings target two audiences: the 2.8$\times$ gap is the pedagogical object for the adaptive-sensing community; the 13.4\% MSE reduction is the engineering-decisive object for the hardware-metrology community.
...
\subsection{Positioning: what we do and do not claim}

We do \emph{not} claim that Bellman's equation for POMDPs is novel, this dates to Bellman~\cite{bellman1957dynamic}, Sondik~\cite{sondik1971optimal}, and Smallwood--Sondik~\cite{smallwood1973sondik}; nor that mutual information as a design reward is novel (Lindley 1956~\cite{lindley1956measure}); nor that joint hardware--policy optimization is novel in general (end-to-end optics already does this for \emph{one-step} reconstructions).  Our novelty is the \emph{joint} optimization with \emph{continuous} physical hardware and a \emph{multi-step Bellman-optimal} adaptive policy, enabled by the envelope theorem and a principled relaxation hierarchy.

We also do \emph{not} claim that the Expected Value of Perfect Information (EVPI)~\cite{howard1966decision, raiffa1961applied} is new, nor that the observation ``EVPI is an upper bound on adaptive gain'' is new.  Our contribution on the EVPI front is the numerically-backed empirical demonstration that $\argmax_{\bc} \E[\mathrm{IG}](\bc) \ne \argmax_{\bc} V_{\text{adaptive}}(\bc)$, with a measured factor of $2.8\times$ gap in attainable adaptive value on a canonical benchmark, and the corresponding observation that EVPI is a useful \emph{diagnostic} but not a replacement for the joint solve.

For clarity the paper avoids the following overclaims that a careful reader will reject on sight: ``there is no Bellman equation for continuous actions'' (there is, one uses local gradient inner optimization, differential dynamic programming, or sample-based methods inside the Bellman backup~\cite{jacobson1970ddp, silver2010monte, munos2014bandits}); ``Fisher information bounds are rigorous for finite-horizon Bayesian estimators'' (the Cram\'er--Rao bound is asymptotic and frequentist~\cite{vantrees2007bayesian, tsang2016quantum}); ``photonic information bounds are redundant given our framework'' (the two are complementary, information bounds characterize the best achievable $\Phi$ at \emph{any} policy; our framework produces the policy that realizes the bound, when the bound is tight).

\subsection{Relation to photonic information-bound and quantum-Fisher-information programs}

Our framework is complementary to two adjacent research programs whose intersection with this work merits explicit discussion.  The first is the recent body of work on fundamental limits to light--matter interaction via convex-duality and T-matrix methods~\cite{miller2016fundamental, miller2023bounds, chao2022maximum, molesky2020tmatrix, molesky2020hierarchical, molesky2020bounds, kuang2020maximal, strekalov2022bounds, gustafsson2020upper, schab2020bounds, strekalov2020bounds, molesky2019global, venkataram2020fundamental, jelinek2021upper}.  These bounds characterize the maximum scattering, absorption, radiative heat transfer, or local density of states achievable by \emph{any} passive physical medium respecting electromagnetic reciprocity and passivity.  Our framework sits downstream: given a family of physical media $\{\bc\}$, we ask how to co-optimize $\bc$ with a Bellman-optimal measurement policy.  An information bound tells you \emph{whether} a target $\Phi$ is achievable in principle; our framework tells you \emph{which} $\bc$ in a feasible family realizes it along with the policy that achieves it.

The second is the body of work on quantum Fisher information for wave and photon scattering~\cite{rotter2017light, bouchet2021maximum, carrington2020fisher, defienne2021coincidence, tsang2016quantum, tsang2017ziv, tsang2019resolving}.  That program establishes Fisher-information optimality of specific wave states with respect to specific estimation tasks, and has developed a rigorous vocabulary for what ``the most informative probe'' means in the wave-scattering and quantum-metrology settings.  Our $\Phi_{\log J}$ choice (used in Case~Study~C below) is the natural inheritor of this language: in the Gaussian-posterior extended-Kalman-filter regime, $\Phi_{\log J}(\bx, \bs, \bc) = \tfrac{1}{2} \log \det J_N(\bx, \bs, \bc)$ is precisely the log-volume of the Bayesian Fisher-information matrix.  In this regime our joint-DP machinery reduces to the Bayesian analog of the maximum-Fisher-information-state calculation; outside this regime ($K$ small, bounded-support priors, biased Bayesian estimators) the Fisher-information path is one of several reasonable scalar rewards, not the uniquely correct one.  The paper's $\Phi$-flexibility thesis is designed precisely to accommodate this kind of pluralism across communities.

\subsection{Organization}

The paper is organized as follows.  Section~\ref{sec:setup} formalizes the joint-$(\bc, \pi)$ optimization.  Section~\ref{sec:phi_family} introduces the family of pointwise scalar rewards $\Phi$ and discusses each member's operational semantics.  Section~\ref{sec:decomposition} introduces the standard $V_{\text{oracle}}/V_{\text{fixed}}/V_{\text{adaptive}}/\E[\mathrm{IG}]$ decomposition.  Section~\ref{sec:envelope} derives the envelope-theorem outer gradient.  Section~\ref{sec:hierarchy} catalogues the relaxation hierarchy.  Sections~\ref{sec:radar}, \ref{sec:scqubit}, and~\ref{sec:photonic} present the three case studies.  Section~\ref{sec:discussion} discusses limitations, $\Phi$-alignment ablations, and the role of EVPI in the design workflow.  Section~\ref{sec:conclusion} concludes.
...
A \emph{sensor} in the sense of this paper is a tuple $(\bc, \pi, \hat\bx)$:
\begin{itemize}[nosep]
\item $\bc \in \mathcal{C} \subseteq \R^{d_c}$, continuous \emph{physical geometry} fixed at manufacturing time.  Examples: the flux-bias and readout-line mutual inductances of a SQUID loop, the permittivity distribution of an inverse-designed metasurface, the elements of a phased-array antenna.
\item $\pi: \mathcal{B} \to \mathcal{S}$, a \emph{measurement policy} that maps the current belief $\bb \in \mathcal{B}$ over the unknown state $\bx$ to an action $\bs \in \mathcal{S}$.  For non-adaptive (``fixed-schedule'') policies $\pi$ is a constant function; for adaptive policies $\pi$ depends on the history of observations.
\item $\hat\bx: \mathcal{B} \to \R^{d_x}$, a \emph{Bayesian point estimator}, conventionally the posterior mean $\hat\bx(\bb) = \mathbb{E}_{\bb}[\bx]$.
\end{itemize}

The hardware $\bc$ parametrizes the likelihood $p(\by_k \mid \bx, \bs_k; \bc)$ through its physical forward model (Maxwell's equations, Hamiltonian dynamics, etc.).  The policy selects $\bs_k$ online based on the history $\mathcal{F}_{k-1} = \{\bs_{1:k-1}, \by_{1:k-1}\}$.  After $K$ decision epochs the sensor returns the point estimate $\hat\bx(\bb_K)$, and the deployment-time cost is the Bayesian MSE
\begin{equation}
  \overline{\mathrm{MSE}}(\bc, \pi) \;=\; \E_{\bx \sim p_0}\E_{\by_{1:K}\mid \bx, \pi, \bc}\!\left[\|\hat\bx(\bb_K) - \bx\|_2^2\right].
\end{equation}

\subsection{Joint optimization statement}

The joint hardware--policy optimization seeks
\begin{equation}
  (\bc^\star, \pi^\star) \;=\; \argmin_{\bc \in \mathcal{C},\; \pi \in \Pi}\; \overline{\mathrm{MSE}}(\bc, \pi).
  \label{eq:joint_opt}
\end{equation}
Equivalently, for any strictly-decreasing $g$, the joint optimization maximizes a scalar $V(\bc, \pi) = -g(\overline{\mathrm{MSE}}(\bc, \pi))$; we will construct such scalars from pointwise functionals $\Phi$ in the next section.

Two degenerate cases recover the classical programs:
\begin{itemize}[nosep]
\item $\Pi = \{\text{constant policies}\}$ and $\hat\bx = $ learned neural reconstruction: \textbf{end-to-end optics}~\cite{sitzmann2018e2e, chang2019diffractive}.
\item $\mathcal{C} = \{\bc_0\}$ (fixed hardware), $\Pi = $ all measurable policies: \textbf{adaptive sensor management}~\cite{kaelbling1998pomdp, kreucher2005kastella}.
\end{itemize}
Their pairing is the content of this paper.

\subsection{Notation}

We use $K$ for the horizon, $d_c$ for the geometry dimension, $\bb$ for the belief, $\pi^\star$ for the Bellman-optimal policy at fixed $\bc$, and $\Phi$ for the pointwise scalar reward.  Inner expectations are over observations $\by$ (with $\bx$ and $\bc$ held fixed); outer expectations are over the prior $p_0(\bx)$.  When a quantity is computed under the optimal policy at given $\bc$ we write $V^\star(\bc) = V(\bc, \pi^\star(\bc))$.  Throughout the paper ``Fisher information'' refers to the Bayesian Fisher information matrix~\cite{vantrees2007bayesian} unless explicitly qualified as ``classical'' or ``frequentist.''

%===============================================================================
\section{The family of pointwise scalar rewards $\Phi$}
\label{sec:phi_family}
%===============================================================================

\subsection{Terminal reward as a modeling choice}

All joint-$(\bc, \pi)$ formulations reduce to maximizing an outer expectation of some scalar terminal functional of the belief:
\begin{equation}
  V(\bc, \pi) \;=\; \E_{\bx, \by_{1:K}}[\, W_K(\bb_K; \bx, \bc) \,].
  \label{eq:V_from_W}
\end{equation}
The terminal functional $W_K$ is a modeling choice.  Three canonical functionals emerge naturally from three different bodies of theory:

\paragraph{Mutual-information-aligned reward $\Phi_{\text{MI}}$.}  Following Lindley~\cite{lindley1956measure}, Chaloner--Verdinelli~\cite{chaloner1995bayesian}, Sebastiani--Wynn~\cite{sebastiani2000optimal}, and the adaptive sensor-management literature~\cite{kreucher2005kastella, hero2011survey},
\begin{equation}
  W_K^{\text{MI}}(\bb_K) \;=\; \ln|\mathrm{supp}(\bb_K)| - H(\bb_K),
  \qquad
  \Phi_{\text{MI}}(\bx, \bs, \bc) \;=\; \E_{\by\mid\bx, \bs, \bc}[\, W_K^{\text{MI}}(\bb_K) \,].
\end{equation}
Summing over $\bx$ under the prior gives $\E_{\bx}[\Phi_{\text{MI}}] = I(\bx; \by_{1:K} \mid \bs, \bc)$, the mutual information.  This is the reward of choice for discrete-state problems where MSE is undefined or dominated by 0/1 classification loss~\cite{castanon2008stochastic}, and for any problem in the large-$K$ Bernstein--von Mises regime~\cite{vaart2000asymptotic, clark2021bvm} where the Gaussian-posterior approximation makes MI monotone in MSE.

\paragraph{Bayesian-MSE-aligned reward $\Phi_{\text{Var}}$.}  Using the tower law of variance,
\begin{equation}
  \E_{\bx, \by_{1:K}}\!\left[(\hat\bx_{\text{pm}} - \bx)^2\right]
  \;=\; \E_{\by_{1:K}}\!\left[\mathrm{Var}(\bx \mid \by_{1:K})\right],
\end{equation}
where $\hat\bx_{\text{pm}}$ is the posterior mean.  Taking
\begin{equation}
  W_K^{\text{Var}}(\bb_K) \;=\; -\mathrm{Var}(\bx \mid \bb_K),
\end{equation}
the outer objective $V(\bc, \pi)$ equals $-1\times$ the deployed Bayesian MSE exactly.  This reward's joint-DP optimum is literally the \emph{MSE-optimal} adaptive policy at fixed $\bc$, without any asymptotic approximation.  For continuous-state, finite-$K$ problems with bounded-support priors where the Bernstein--von Mises regime is not reached, $\Phi_{\text{Var}}$ strictly dominates $\Phi_{\text{MI}}$ as an MSE anchor.

\paragraph{Fisher-information-aligned reward $\Phi_{\log J}$.}  For continuous $\bx$ with a Gaussian-posterior surrogate, extending the Bayesian Cram\'er--Rao~\cite{vantrees2007bayesian, tichavsky1998posterior} tradition,
\begin{equation}
  W_K^{\log J}(\bb_K, \bx) \;=\; \tfrac{1}{2} \log\det J_N(\bx, \bs_{1:K}, \bc),
\end{equation}
where $J_N$ is the accumulated Fisher information matrix at true $\bx$.  This reward's outer expectation is the log-determinant of the Bayesian Fisher information matrix~\cite{pilanci2015randomized, chaloner1995bayesian}, and its inverse $J_N^{-1}$ is the Bayesian CRB.  In the Gaussian-posterior regime $\Phi_{\log J}$ agrees with $\Phi_{\text{Var}}$ up to log-transforms.  Outside that regime, biased estimators, bounded priors, small $K$, $\Phi_{\log J}$ retains an asymptotic CRB interpretation but loses its direct link to finite-sample MSE.  It is the natural reward for high-dimensional continuous problems where the full Bellman DP is not tractable and the Gaussian surrogate is the cheapest defensible relaxation.
...
\begin{tabular}{@{}llll@{}}
\toprule
reward & leaf value $W_K$ & community & tied to pointwise MSE? \\
\midrule
$\Phi_{\text{MI}}$  & $\ln|\mathrm{supp}| - H(\bb_K)$                   & sensor management, sOED & asymptotic only \\
...
\subsection{Connection to classical and quantum information bounds}

Each of the three rewards connects cleanly to a body of prior work:
\begin{itemize}[nosep, leftmargin=2em]
\item $\Phi_{\text{MI}}$ inherits from Shannon's mutual information~\cite{shannon1948mathematical} through Lindley's Bayesian decision theory~\cite{lindley1956measure}.  The photonic and quantum-sensing communities have developed extensive machinery for \emph{upper bounds} on achievable mutual information given physical constraints (reciprocity, passivity, energy); our framework consumes those bounds as constraints on the feasible set $\{\Phi_{\text{MI}}(\cdot, \cdot, \bc) : \bc \in \mathcal{C}\}$ rather than asserting new bounds.
\item $\Phi_{\text{Var}}$ inherits from the Bayesian Cram\'er--Rao bound~\cite{vantrees2007bayesian} and the Bayesian posterior-variance-minimization tradition in Bayesian optimal experimental design~\cite{chaloner1995bayesian, pronzato2010optimal}.  The tower-law identity is textbook; its use as a joint-DP terminal reward is not.
\item $\Phi_{\log J}$ inherits from the classical Fisher-information-matrix tradition~\cite{cramer1946mathematical, rao1945information, berger1985statistical} and its Bayesian extension~\cite{vantrees2007bayesian, tichavsky1998posterior}.  The quantum-Fisher-information program on wave scattering~\cite{bouchet2021maximum, tsang2016quantum, rotter2017light, carrington2020fisher} and the photonic information-bound program on scattering-matrix extremization~\cite{miller2016fundamental, miller2023bounds, chao2022maximum, molesky2020tmatrix, molesky2020hierarchical} have established upper bounds on $\det J_N$ over admissible $\bc$.  These bounds complement our framework: they tell you what $\sup_{\bc} \det J_N$ is; our framework tells you which $\bc$ and which policy realize it jointly.
\end{itemize}

%===============================================================================
\section{The $V_{\text{oracle}} / V_{\text{fixed}} / V_{\text{adaptive}} / \E[\mathrm{IG}]$ decomposition}
...
\subsection{Chain of inequalities}
...
\label{eq:chain}
\end{equation}
The left inequality is trivial (fixed schedules are a subset of adaptive policies).  The right inequality is the EVPI bound: no adaptive policy can do better than an oracle who knows $\bx$ outright.  Rearranging,
\begin{equation}
V_{\text{adaptive}}(\bc) - V_{\text{fixed}}(\bc) \;\le\; \E[\mathrm{IG}](\bc).
\label{eq:evpi}
\end{equation}
This is the standard EVPI upper bound on adaptive gain.

\subsection{The EVPI is loose: a central empirical fact}

The critical empirical observation that motivates the paper, and which we will back up quantitatively in Section~\ref{sec:radar}, is that the inequality in~\eqref{eq:evpi} is typically \emph{slack}, and the slackness pattern is non-monotone in $\bc$:
\begin{equation}
\argmax_{\bc} \,\E[\mathrm{IG}](\bc) \;\;\ne\;\; \argmax_{\bc} V_{\text{adaptive}}(\bc).
\label{eq:argmax_mismatch}
\end{equation}
In words, the geometry that maximizes the information bound (the pre-screen any statistician's first instinct would reach for) is typically \emph{not} the geometry that maximizes the attainable adaptive value.  The EVPI is therefore useful as a cheap diagnostic, it upper-bounds the adaptive slack at each geometry, but is not a substitute for the full joint solve.

\subsection{EVPI's operational role, calibrated}

The EVPI remains useful in three roles in the design workflow:
\begin{enumerate}[nosep]
\item \textbf{Cheap pre-screen.}  Computing $\E[\mathrm{IG}](\bc)$ requires $V_{\text{oracle}}$ (enumeration over schedules at known $\bx$) and $V_{\text{fixed}}$ (enumeration at unknown $\bx$).  Both are much cheaper than the full Bellman DP for $V_{\text{adaptive}}$.  When the full DP is prohibitive, EVPI alone provides a defensible upper bound on adaptive reach.
\item \textbf{Post-hoc interpretation.}  After solving the joint problem, $\E[\mathrm{IG}](\bc^\star)$ is the headroom between the adaptive value and the oracle ceiling, useful for understanding where further adaptive gains could come from (longer horizon, richer action set, etc.).
\item \textbf{Diagnostic lens on the slackness pattern.}  When $\argmax_{\bc} \E[\mathrm{IG}](\bc)$ and $\argmax_{\bc} V_{\text{adaptive}}(\bc)$ disagree, as in~\eqref{eq:argmax_mismatch}, the disagreement structure exposes the physics: extreme geometries typically have the largest EVPI because they leave the most adaptive headroom on the table; interior geometries capture more of a smaller ceiling.  Section~\ref{sec:radar} develops this in detail.
\end{enumerate}

We emphatically do \emph{not} recommend using $\argmax_{\bc} \E[\mathrm{IG}](\bc)$ as a design target when the full joint solve is available; Case~Study~A below shows this costs $2.8\times$ in attainable adaptive value on a canonical benchmark.
...
\begin{equation}
W_k(\bb) = \max_{\bs \in \mathcal{S}} \E_{\by}[W_{k+1}(T(\bb, \bs, \by))].
\end{equation}
...
The outer loop is the policy iteration of Section~\ref{sec:envelope}.  At each $\bc$ the inner sensor-parameter argmax $\bs^\star(\bb_k; \bc)$ is obtained by grid search + L-BFGS.  Its derivative with respect to $\bc$ is required by the chain rule; we supply it via a custom \texttt{rrule} that uses the implicit function theorem at the inner argmax (bypassing the need for nested automatic differentiation).  The outer Adam optimizer uses a density filter of radius $R = 5$~pixels and a hyperbolic-tangent threshold projection with continuation parameter $\beta$ annealed from $\beta = 16$ to $\beta = 256$ to enforce fabrication constraints and near-binary permittivity~\cite{wang2011projection, sigmund2007morphology, lazarov2016length, christiansen2018compact}.  The $\beta$ schedule is steered by an autonomous tuning agent that reads inspector snapshots of the geometry, loss-trajectory, and gradient distribution after each cron-scheduled fire and writes a control file consumed by the Julia process on the next iteration~\cite{autotune2026}.

\subsection{Results at the manufacturable $\beta = 256$ binary design}

The autotune run starts from a random permittivity initialization and completes 583 outer Adam iterations through a $\beta$ continuation schedule $\beta \in \{16, 32, 64, 128, 256\}$.  Key milestones:
\begin{itemize}[nosep]
\item Moving-average-20 loss drops from $\sim\!2.5$ at initialization to $5.15 \times 10^{-4}$ at the terminal $\beta = 256$ plateau (loss minimum $1.63 \times 10^{-4}$).
\item Binary percentage of the projected permittivity increases monotonically through the $\beta$ schedule: $10.6\% \to 43\% \to 69.6\% \to 84.8\% \to \mathbf{92.4\%}$ at $\beta = 256$.  The terminal 92.4\% reflects a structural ceiling of the density filter at $R = 5$: the remaining $\sim 7.6\%$ gray pixels concentrate at thin curvilinear interfaces that the filter cannot sharpen further without reducing $R$.
\item Post-$\beta$-doubling loss spikes decrease progressively as the geometry pre-commits: $159\times$ at $\beta = 32$, $230\times$ at $\beta = 64$, $30\times$ at $\beta = 128$, and \emph{no spike} at $\beta = 256$.  Each spike is followed by clean exponential recovery over 40--80 iterations.  Isolated-pixel fraction remains at the run-minimum $(0.004\text{--}0.007)\%$ throughout, and the geometry is free of wrong-basin blobs (\texttt{grad\_sat\_pct} never exceeds noise).
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/photonic_geometry_beta256.png}
\caption{The $\beta = 256$ manufacturable photonic geometry at the end of the $583$-iteration autotune run.  Left: raw permittivity field.  Middle: density-filtered permittivity at filter radius $R = 5$ pixels.  Right: tanh-threshold projection at $\beta = 256$; terminal binary fraction is $92.4\%$, with the remaining $\sim 7.6\%$ gray pixels concentrated at thin curvilinear interfaces that the filter cannot sharpen further without reducing $R$.}
\label{fig:photonic_geometry}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/photonic_loss_trajectory.png}
\caption{Loss trajectory through the $\beta \in \{16, 32, 64, 128, 256\}$ continuation schedule.  Post-doubling loss spikes decrease progressively as the geometry pre-commits: $159\times$, $230\times$, $30\times$, and no spike at $\beta = 256$.  Each spike is followed by clean exponential recovery over $40$--$80$ iterations.  Terminal moving-average-20 loss is $5.15 \times 10^{-4}$; loss minimum is $1.63 \times 10^{-4}$.}
\label{fig:photonic_loss}
\end{figure}

\subsection{Monte-Carlo EKF deployment at the $\beta = 256$ optimum}

Deploying the $\beta = 256$ binary design with the EKF reconstruction and fresh perturbation draws (200 episodes, $K = 10$ EKF steps per episode):

\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
metric & optimized ($\beta = 256$) & random initial & ratio \\
\midrule
...
\begin{enumerate}[nosep]
\item A cheap pre-screen when the full joint DP is computationally prohibitive.  $\E[\mathrm{IG}](\bc)$ requires only $V_{\text{oracle}}$ and $V_{\text{fixed}}$, both computable by direct schedule enumeration without backward induction.
