...
We run the policy iteration of Section~\ref{sec:envelope} with outer Adam updates at learning rate $5 \times 10^{-4}$ and periodic policy reoptimization, on a physically realistic restricted geometry box ($f_q^{\max} \in [3, 12]$~GHz, $E_C/h \in [0.15, 0.4]$~GHz, $\kappa \in [0.1, 5]$~MHz, $\Delta_{qr} \in [0.8, 5]$~GHz, noise amplitudes and temperature pinned at paper-baseline values).  The outer Adam uses per-component box-width preconditioning (a minor but load-bearing implementation detail; without it the outer step sizes span 14 orders of magnitude and stall geometric exploration).  Best-c-seen over the trajectory:
\begin{equation}
...
\end{equation}
with $\bs^\star_2 = (\tau_{10}, n_2)^4 = (320~\mathrm{ns}, 10)^4$.  Both $\bc^\star_1$ and $\bc^\star_2$ are numerically optimized under their respective objectives and end up at materially different points in the restricted box: the deployed-MSE gap below is a full co-design vs.\ co-design comparison, not a shared-geometry policy-structure test.
...
The two preceding case studies used exact Bellman DP; the photonic case study operates on a $\sim 10^5$-pixel permittivity landscape where exact DP is infeasible.  We demonstrate the relaxation-hierarchy rung 4: Gaussian-posterior with extended-Kalman-filter updates, Bayesian Fisher-information-matrix (BFIM) reward $\Phi_{\log J}$, and implicit-function-theorem differentiation through the inner sensor-parameter argmax.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/photonic_hero.png}
\caption{Case Study C overview.  \emph{Left}: deployed-MSE comparison between the joint-DP co-designed 90{,}000-pixel permittivity landscape (MSE $= 9.8 \times 10^{-4}$) and a non-co-designed baseline at the same physical parameters (MSE $= 1.2 \times 10^{-1}$), a $123\times$ reduction; the BFIM surrogate $\tfrac{1}{2}\log\det J_N$ reaches $24.6$~nats at the final $\beta = 256$ checkpoint.  \emph{Right}: the $300 \times 300$ binary permittivity pattern (silicon dark, cladding light) after co-optimization, with four waveguide ports (N, S, E, W) at the slab edges; the two input phases $(\phi_1, \phi_2)$ are the adaptive sensor-parameter argmax, differentiated through via the implicit-function-theorem envelope gradient (§6, rung 4).}
