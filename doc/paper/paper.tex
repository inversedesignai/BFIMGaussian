...
\paragraph{Dichotomy at the exact-DP branch point.}  The dominant approach in modern reinforcement learning is to parametrize the policy $\pi_\theta$ (or the action-value $Q_\theta$) with a neural network and optimize it by policy gradient, $Q$-learning, or actor--critic~\cite{hausknecht2015deep, zhong2018deep}.  \emph{We do not pursue this branch in the present paper, though we do not rule it out for future work.}  The reason is a well-documented pathology: an expressive NN policy can absorb the training signal and compensate for a poor $\bc$ by investing in policy complexity, leaving $\bc$ near its initialization.  The outer loop converges to a \emph{hardware-light} fixed point that looks good in simulation but relies on a near-trivial $\bc$ hard to justify as a physical design.  Known countermeasures --- capacity control on $\pi_\theta$ (bottleneck width, weight decay, dropout), two-timescale updates with $\bc$ stepped on a faster clock than $\theta$, warm-starts from one of the prescribed inner rules below (rungs 3--4), and auxiliary regularizers penalizing hardware-null geometries --- partially mitigate the risk but do not yet admit the clean envelope-theorem outer gradient our framework exploits.  We flag the combination of these remedies as a plausible path to extending joint-$(\bc, \pi)$ to expressive NN policies in future work, and correspondingly exclude NN-parametrized policies from the hierarchy below on a \emph{provisional} rather than principled basis.  Parametrizing the value function alone with a neural network while keeping an explicit Bayesian filter as the inference primitive (belief-based deep $Q$-learning) is a partial workaround --- it retains the inference-explicit structure --- but inherits a milder form of the same risk whenever the value-network capacity is large enough to mask hardware mis-specification.

\paragraph{The explicit-inference cascade (our path).}  The alternative, which we pursue, is to \emph{prescribe} $\pi$ in closed form as an information-theoretic rule that approximates Bellman-optimality to the extent affordable, and to focus the optimization machinery on $\bc$.  Each rule consumes an \emph{explicit} inference primitive (a belief, its moments, or a point estimate) maintained by a Bayesian filter; the envelope theorem on the outer gradient then eliminates the policy Jacobian and yields a clean $\bc$-derivative at the inner $\argmax$.  This philosophy does not attempt to optimize the policy directly; it picks a principled heuristic that is plausibly close to the Bellman optimum at its feasibility ceiling, then uses the outer loop to move $\bc$.  The hierarchy is a \emph{cascade}: each rung is triggered by the previous becoming intractable.

A structural feature of this cascade, shared with exact DP (rung 1) and sharply distinguishing it from the NN branch above, is that \emph{inference is maintained explicitly by a Bayesian filter at every rung.}  What descends the cascade is (i) the policy's proximity to the Bellman optimum --- Bellman-optimal at rung 1, clairvoyant-CE at rung 2, myopic one-step at rungs 3--4 --- and (ii) the coarseness of the belief representation the filter maintains --- exact posterior, point estimate, full distribution, Gaussian moments.  At no rung is the inference primitive replaced by a learned encoder over the raw observation history; the Bayesian filter is a first-class object throughout.  The envelope-theorem gradient of Section~\ref{sec:envelope} relies on this: it applies whenever the inner rule is an $\argmax$ over an inference-explicit state, regardless of how coarsely that state approximates the true posterior.
