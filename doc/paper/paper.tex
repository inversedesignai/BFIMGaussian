...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/radar_hero.png}
\caption{Case Study A overview.  \emph{Left}: the in-family optimum $m^\star = 9$ achieves $V_{\text{adaptive}} = 1.704$ nats against $V_{\text{oracle}} = 1.879$ nats and $V_{\text{fixed}} = 1.401$ nats; the EVPI ($0.478$ nats) is the oracle-fixed ignorance gap that the adaptive policy recovers a fraction of.  \emph{Right}: a $16$-element phased-array antenna (16-element $\lambda/2$ array) interrogates a ring of $K = 16$ target cells; the true target (red, cell 4), the current belief mode (green, cell 7), the current beam pointing at width $m = 3$, and the next-step adaptive redirect under $\pi^\star$.}
\label{fig:radar_scene}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/scqubit_hero.png}
\caption{Case Study B overview.  \emph{Left}: the deployed-MSE comparison at the uninformative prior ($\phi_\mathrm{max} = \Phi_0/2$): joint-DP co-design $\overline{\mathrm{MSE}} = 1.83 \times 10^{-2}\,\Phi_0^2$ vs.\ PCRB baseline $2.07 \times 10^{-2}\,\Phi_0^2$ ($13.4\%$ reduction, $z = +11.5\sigma$); the advantage grows monotonically to $8.3\times$ at $\phi_\mathrm{max} = \Phi_0/10$ and $128\times$ at $\phi_\mathrm{max} = \Phi_0/33$ (§8.7.3).  \emph{Right}: the x-mon transmon cross on sapphire with a SQUID loop (two Josephson junctions, red dots) at the end of the $+x$ arm, a flux-bias line (purple) threading $\Phi_\mathrm{ext}$ (navy field lines) through the SQUID, and a meandering readout resonator (green); the belief evolution $p_0(x) \to p_1(x) \to p_K(x)$ (right inset curves) narrows to a sharp posterior delivering the MSE-optimal estimate $\hat{x}^\star$; the adaptive schedule $s_k = (\tau_k, n_k)$ is chosen by $\pi^\star(b_k)$ each epoch.}
\label{fig:scqubit_scene}
\end{figure}
...
\subsection{Prior-width dependence: from $1.1\times$ to $128\times$}
\label{sec:scqubit_priorwidth}

The $13.4\%$ headline above is at the maximally uninformative prior $\Phi_\mathrm{ext} \sim \mathrm{Uniform}[0, \Phi_0/2]$.  Many practical flux sensors operate around a pre-tuned working point and the relevant prior spans only a fraction of $\Phi_0$.  The adaptive-over-PCRB gap grows dramatically as the prior narrows.  We ran the same $K = 4, J = 10, L = 2$ config at \textsc{PAPER\_BASELINE} $\bc$ (no outer geometry optimization) at $K_\Phi = 256$ deployed grid, $n_{\mathrm{mc}} = 20{,}000$ trajectories:

\begin{center}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
$\phi_{\max}$ (units of $\Phi_0$) & $\overline{\mathrm{MSE}}_1$ (joint-DP) & $\overline{\mathrm{MSE}}_2$ (PCRB) & ratio & $z$-score & prior variance \\
\midrule
$0.03$ & $5.47 \times 10^{-7}$ & $7.00 \times 10^{-5}$ & $\mathbf{128.0\times}$ & $+151\sigma$ & $7.5 \times 10^{-5}$ \\
$0.05$ & $2.65 \times 10^{-6}$ & $2.04 \times 10^{-4}$ & $\mathbf{76.9\times}$  & $+155\sigma$ & $2.1 \times 10^{-4}$ \\
$0.08$ & $4.30 \times 10^{-5}$ & $5.35 \times 10^{-4}$ & $\mathbf{12.4\times}$  & $+131\sigma$ & $5.3 \times 10^{-4}$ \\
$0.10$ & $1.02 \times 10^{-4}$ & $8.42 \times 10^{-4}$ & $\mathbf{8.29\times}$  & $+166\sigma$ & $8.3 \times 10^{-4}$ \\
$0.15$ & $5.69 \times 10^{-4}$ & $1.90 \times 10^{-3}$ & $3.33\times$  & $+83\sigma$  & $1.9 \times 10^{-3}$ \\
$0.20$ & $2.38 \times 10^{-3}$ & $3.38 \times 10^{-3}$ & $1.42\times$  & $+28\sigma$  & $3.3 \times 10^{-3}$ \\
$0.25$ & $4.28 \times 10^{-3}$ & $5.33 \times 10^{-3}$ & $1.25\times$  & $+18\sigma$  & $5.2 \times 10^{-3}$ \\
$0.30$ & $7.33 \times 10^{-3}$ & $7.71 \times 10^{-3}$ & $1.05\times$  & $+4\sigma$   & $7.5 \times 10^{-3}$ \\
$0.40$ & $1.54 \times 10^{-2}$ & $1.37 \times 10^{-2}$ & $0.89\times$  & $-10\sigma$  & $1.3 \times 10^{-2}$ \\
$0.49$ & $2.53 \times 10^{-2}$ & $2.04 \times 10^{-2}$ & $0.81\times$  & $-19\sigma$  & $2.0 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\end{center}

\vspace{4pt}
\noindent The $50\%$-gap threshold is crossed between $\phi_{\max} = 0.20$ and $\phi_{\max} = 0.15$; at $\phi_{\max} = 0.10$ the gap is $\mathbf{8.3\times}$ ($729\%$) and at $\phi_{\max} = 0.03$ it reaches $\mathbf{128\times}$ (the deployed-MSE equivalent of the paper's $123\times$ photonic headline, achieved here without outer geometry optimization).  At $\phi_{\max} = 0.49$ (full support) the PCRB schedule marginally beats the joint-DP policy because both are aliasing-saturated near the prior variance ($\overline{\mathrm{MSE}} \approx \Phi_0^2/48$) and the adaptive DP's short-$\tau$ disambiguation cannot unwind $\sim\!1600$ Ramsey fringes at $K = 4$.

\paragraph{Mechanism.}  At $\tau = 320$~ns and $f_q \approx 9$~GHz the Ramsey-fringe period in $\Phi_\mathrm{ext}$ is $\Delta\Phi \approx 3 \times 10^{-4}\,\Phi_0$.  A wide prior spans $O(10^3)$ fringes that $K = 4$ cannot untangle; a narrow prior spans $O(10^2)$ fringes, few enough for the adaptive policy's root action --- in our runs, $\tau \approx 22$~ns --- to pin a single fringe before the remaining $K - 1$ epochs sharpen within it.  The PCRB-optimal schedule is fixed at $(\tau_{\max}, n_{\max})^K$ in both regimes, saturating at the prior variance regardless of $\phi_{\max}$.  The joint-DP advantage at narrow prior is therefore a pure \emph{policy-structure} effect: same hardware, same shot budget, but a belief-conditional schedule.  Outer geometry optimization adds a further $\sim\!20\%$ of variance reduction but is not the primary lever --- the lever is the adaptive policy's short-$\tau$ disambiguation at narrow prior, which PCRB cannot replicate at any $\bc$.

\paragraph{Operational implication.}  For flux sensors with a known rough bias (the typical deployment), the narrow-prior regime is exactly where joint-DP co-design pays off most; the quoted $13.4\%$ headline is a \emph{conservative lower bound} reflecting the maximally pessimistic uninformative prior.  In practice a well-instrumented sensor with prior width $\phi_{\max} \lesssim 0.1\,\Phi_0$ will see adaptive advantages of one-to-two orders of magnitude.
