...
\caption{Scene illustrations for the two case studies.  \emph{Left} (Case Study A, §\ref{sec:radar}): a phased-array antenna interrogates a ring of $K = 16$ target cells; the true state (green, cell 11), the current beam pointing at cell $\bs_k = 5$ with width $m = 3$, and the adaptive redirect selected by $\pi^\star$ at the next epoch.  \emph{Right} (Case Study B, §\ref{sec:scqubit}): the x-mon transmon cross on sapphire with a SQUID loop (two Josephson junctions) at the end of the $+x$ arm, the external flux $\Phi_\mathrm{ext}$ (navy field lines) threading the loop, a meandering readout resonator, and the Bloch-sphere view of the qubit; the bottom timeline shows the adaptive Ramsey sequences $\bs_k = (\tau_k, n_k)$ chosen by $\pi^\star(\bb_k)$ at each epoch.}
\label{fig:casesAB}
\end{figure}
