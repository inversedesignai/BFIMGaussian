...
The joint $(\bc, \pi)$ optimization as stated in Section~\ref{sec:setup} is exactly solvable only for small discrete POMDPs.  In practice one must relax either the inner Bellman solver, the state-space representation, or both.  We organize the relaxations along the axis that matters most for the rest of our analysis: \emph{where does the inference step enter}?  Each rung in the hierarchy keeps an explicit \emph{belief} $\bb_k = p(\bx \mid \mathcal{F}_{k-1}; \bc)$ as the state the inner rule consumes, varying only in how the belief is represented (exact grid, point estimate, full distribution, Gaussian moments).  This is what makes the oracle / ignorance-gap / Fisher-trace analyses in Sections~\ref{sec:phi_family}--\ref{sec:decomposition} work uniformly across rungs.  We call this axis \emph{inference-explicit}.  A separate, non-hierarchical branch (history-based deep RL with learned encoders) is inference-implicit; we discuss it at the end of the section.

\begin{center}
\begin{tabular}{@{}llll@{}}
\toprule
rung & method & inference primitive (belief representation) & failure mode \\
\midrule
1 & Exact Bellman DP                        & exact belief (grid or tabulated)    & memoization $\sim|\mathcal{S}|^K$ for $K \gtrsim 6$ \\
2 & Approximate DP / belief-based deep RL   & NN-$Q$ or NN-$V$ on explicit belief & value-function approximation error \\
3 & Certainty-equivalent control            & posterior point estimate $\hat\bx$  & under-exploration; ignores posterior spread \\
4 & Myopic Bayesian $D$-optimal             & full belief $\bb_k$, one-step look-ahead & ignores future decisions \\
5 & Gaussian-trace + EKF                    & Gaussian belief $(\bar\bx, \Sigma)$  & Gaussian-posterior approximation breaks near bounds \\
\bottomrule
\end{tabular}
\end{center}

The three case studies below cover rungs 1, 1, and 5 respectively.  Rungs 2--4 are consumed from the literature: POMCP~\cite{silver2010monte}, belief-based deep $Q$-learning~\cite{hausknecht2015deep}, deep RL for sensor scheduling~\cite{zhong2018deep}, Bayesian optimal experimental design with adjoint surrogates~\cite{huan2016sequential, shen2023bayesian}, and myopic expected information gain~\cite{drovandi2014sequential}.  They integrate into the framework via the envelope theorem without modification because they share the inference-explicit structure of the inner rule.

\paragraph{Inference-implicit branch (orthogonal to the hierarchy).}  A large fraction of modern deep reinforcement learning on partially observed environments bypasses the inference primitive entirely: an LSTM, GRU, or attention encoder over the raw history $\mathcal{F}_{k-1}$ supplies a learned latent state to a value or policy network (DRQN~\cite{hausknecht2015deep}, recurrent PPO, transformer-based RL).  These methods do not maintain $\bb_k$ as an explicit object; whatever belief-like representation emerges is a side-effect of end-to-end training.  We do not include them as rungs of our hierarchy because the oracle / ignorance-gap / Fisher-trace analyses require $\bb_k$ as a first-class quantity, and because the envelope-theorem outer gradient of Section~\ref{sec:envelope} is cleanest when the inner rule is an $\argmax$ over an inference-explicit state.  Inference-implicit RL is a legitimate alternative that trades analytical transparency for scalability in ways orthogonal to the tractability--fidelity trade-offs organized here.
