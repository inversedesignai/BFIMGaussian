...
\subsection{EKF Performance: Optimized vs.\ Random Geometry}

To quantify the effect of geometry optimization on estimation accuracy, we ran 200 Monte Carlo EKF episodes (10 steps each) using the optimized geometry from Section~7.4 and compared against a random (unoptimized) geometry processed through the same density filter and projection pipeline.  Both evaluations use identical parameters ($\sigma^2 = 10^{-10}$, $\Delta n \in [10^{-5}, 10^{-4}]$, $\Sigma_0 = 7 \times 10^{-10} \cdot I$) and adaptive BFIM-based sensor selection at each step.  MLE estimates (L-BFGS on the cumulative likelihood) are computed alongside the EKF for comparison.

\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
Metric & Optimized & Random & Ratio \\
\midrule
EKF final rel.\ error (10 steps) & 0.0097 (0.97\%) & 1.198 (120\%) & $123\times$ \\
Episode loss (3 steps, mean) & $1.48 \times 10^{-4}$ & 2.724 & $18{,}400\times$ \\
BFIM trace (step 1) & $2.50 \times 10^{13}$ & $4.01 \times 10^{10}$ & $622\times$ \\
$\tr(\Sigma)$ final & $1.35 \times 10^{-13}$ & $1.41 \times 10^{-9}$ & $10{,}400\times$ \\
$\tr(\Sigma)$ reduction from prior & $48{,}400\times$ & $2\times$ & \\
EKF vs.\ MLE (final) & $1.01\times$ MLE & $0.055\times$ MLE & \\
Mahalanobis$^2$ (final, expect $\approx 4$) & 0.13 & 3.82 & \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Per-step error reduction:}

\begin{center}
\begin{tabular}{@{}rrrr@{}}
\toprule
Step & Optimized rel.\ err & Random rel.\ err & Improvement \\
\midrule
0 (prior) & 1.84 & 1.84 & $1\times$ \\
1 & 0.034 & 1.25 & $37\times$ \\
3 & 0.018 & 1.22 & $66\times$ \\
5 & 0.014 & 1.21 & $86\times$ \\
10 & 0.0097 & 1.20 & $123\times$ \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Key findings:}
\begin{itemize}[nosep]
  \item The optimized geometry provides $622\times$ more Fisher information per measurement, enabling the EKF to reduce relative error from 184\% to 3.4\% in a single step---essentially solving the estimation problem with one broadband measurement.
  \item The random geometry barely improves over 10 steps (184\% $\to$ 120\%, only 35\% reduction), confirming that geometry optimization---not just adaptive sensor selection---is the dominant factor in estimation performance.
  \item EKF and MLE achieve nearly identical accuracy on the optimized geometry ($1.01\times$), indicating that the Taylor-model linearization is excellent in this regime.  On the random geometry, MLE diverges ($21.9\times$ prior error) while the EKF's prior regularization prevents catastrophic failure.
  \item The optimized EKF is underconfident (Mahalanobis$^2 = 0.13 \ll d_x = 4$): actual errors are much smaller than the posterior covariance predicts.  The random geometry EKF is well-calibrated (Mahalanobis$^2 \approx 3.8$), consistent with a filter that extracts minimal information.
  \item The $18{,}400\times$ improvement in episode loss (the training objective) validates that the bilevel optimization framework successfully translates geometry changes into estimation accuracy gains.
\end{itemize}
...
Monte Carlo evaluation (200 episodes, 10 EKF steps) showed that the optimized geometry provides $622\times$ more Fisher information per measurement than a random geometry and achieves sub-percent relative estimation error ($0.97\%$) after 10 steps---a $123\times$ improvement over the random baseline ($120\%$).  The optimized EKF matches MLE accuracy ($1.01\times$), confirming excellent Taylor-model fidelity, while the random geometry's MLE diverges entirely.  These results demonstrate that geometry optimization is the dominant factor in estimation performance, and that the bilevel framework successfully translates design-region changes into orders-of-magnitude estimation accuracy gains.
