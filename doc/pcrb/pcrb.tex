...
\begin{abstract}
We present the Posterior Cram\'er--Rao Bound (PCRB) in its most general form for sequential Bayesian estimation, following Tichavsk\'y, Muravchik \& Nehorai (1998).  We then specialize to two cases: (i)~a static state observed through nonlinear measurements, and (ii)~additive Gaussian noise with known covariance.  Finally, we formulate the D-optimal joint optimization of sensor parameters and device geometry as a single-level problem over fixed design variables, and bound the potential benefit of an adaptive policy over this fixed design by an \emph{ignorance gap} that is near-zero for weakly nonlinear measurement models.  The adaptive case is developed as a dynamic program in the value function, with the gap-function reformulation and its exploration--exploitation decomposition relegated to an appendix.
\end{abstract}
...
Define the value achieved by the \emph{optimal} adaptive policy:
\begin{equation}
  V_{\text{adaptive}} = \max_{\pi}\;\E_{\bm{x}, \bm{y}}\!\left[\ln\det\bm{J}_N(\bm{x}, \pi(\bm{y}_{1:N-1}), \bm{c})\right],
\end{equation}
where the maximum is over all measurable policies $\pi = \{\pi_k: \bm{y}_{1:k-1} \mapsto \bm{s}_k\}$.  This satisfies:
\begin{equation}
  V_{\text{fixed}} \;\leq\; V_{\text{adaptive}} \;\leq\; \E_{\bm{x}}\!\left[V_{\text{oracle}}(\bm{x})\right],
  \label{eq:bounds}
\end{equation}
where the left inequality holds because the fixed design is a special (constant) policy included in the max, and the right because no policy can beat the oracle.  Therefore:
\begin{equation}
...
When $\E[IG]$ is non-negligible, one may try to capture part of the gap by running an adaptive policy $\pi = \{\pi_k : \bm{y}_{1:k-1} \mapsto \bm{s}_k\}$.  The optimal adaptive policy solves a POMDP on the belief space.  We present its Bellman formulation here; the reformulation in terms of the conditional ignorance gap $\Gamma_k$ (with its exploration--exploitation decomposition and simplifying regimes) is developed in Appendix~\ref{app:gap_dp}.

\paragraph{Setup and notation.}
Write $\Phi(\bm{x}, \bm{s}_{1:N}) = \ln\det\bm{J}_N(\bm{x}, \bm{s}_{1:N}, \bm{c})$ for the log-information at state $\bm{x}$ with sensor sequence $\bm{s}_{1:N}$.  Let $b_k = p(\bm{x} \mid \bm{y}_{1:k-1})$ denote the posterior belief after $k-1$ observations (so $b_1 = p_0$ is the prior).
...
\end{align}
where $b_{k+1}$ is the Bayesian posterior update of $b_k$ given $(\bm{y}_k, \bm{s}_k)$.  The derivation of \eqref{eq:bellman}--\eqref{eq:terminal} from first principles (sufficient statistic reduction, peel-off-one-action, tower property) is in Appendix~\ref{app:bellman}.  This is a POMDP over the continuous belief space---intractable in general---and $V_{\text{adaptive}} = W_1(\text{prior})$.
...
\begin{equation}
  ...
  \label{eq:oracle_vtg}
\end{equation}
Let $V_{k+1}^\star(\bm{x};\bm{s}_k) = \max_{\bm{s}_{k+1:N}}\Phi(\bm{x}, \bm{s}_{1:k-1}, \bm{s}_k, \bm{s}_{k+1:N})$ denote the oracle's value-to-go after committing $\bm{s}_k$.  Then $V_k^\star(\bm{x}) = \max_{\bm{s}_k} V_{k+1}^\star(\bm{x};\bm{s}_k)$, since the oracle observes $\bm{x}$ directly and faces no uncertainty.  The adaptive policy can never exceed the oracle:
\begin{equation}
...
The deficit of the adaptive policy relative to the oracle is the \emph{conditional ignorance gap}:
\begin{equation}
  \boxed{\Gamma_k(b_k) = \E_{\bm{x} \mid b_k}\!\left[V_k^\star(\bm{x})\right] - W_k(b_k) \;\geq\; 0,}
  \label{eq:cond_gap}
\end{equation}
a function of the current belief alone.  At $k = 1$ with the prior, $\Gamma_1(\text{prior}) = \E_{\bm{x}}[V_{\text{oracle}}(\bm{x})] - V_{\text{adaptive}}$ is the residual gap that even the optimal adaptive policy cannot close---bounded in turn by $\E[IG(\bm{x})]$.  Substituting $W_k = \E[V_k^\star] - \Gamma_k$ into the Bellman recursion \eqref{eq:bellman} gives a dynamic program directly on $\Gamma_k$, the \emph{gap DP}, which exposes the exploration--exploitation tradeoff of optimal adaptive sensing.  The full development is in Appendix~\ref{app:gap_dp}.

\subsubsection{Joint optimization of geometry and adaptive policy}
\label{subsec:joint_adaptive}
...
\end{equation}
where $W_1(\text{prior};\, \bm{c})$ is the Bellman-optimal value \eqref{eq:bellman} for geometry $\bm{c}$.  The geometry affects the Jacobians $\bm{F}_k(\bm{x}, \bm{s}, \bm{c})$, the oracle map $\bm{s}^\star(\bm{x};\, \bm{c})$, and (through them) every gap term $\Delta_k$ in Appendix~\ref{app:gap_dp}.
...
\begin{equation}
  ...
  \label{eq:envelope}
\end{equation}
The first-order effect of changing the policy vanishes at the optimum.  One differentiates as if the policy were frozen: only the direct effect of $\bm{c}$ on measurement quality matters, evaluated at whatever sensors the optimal policy happens to choose.

This enables a \textbf{policy iteration} scheme:
\begin{enumerate}[nosep]
  \item \textbf{Policy step}: for current $\bm{c}$, solve or approximate the gap DP of Appendix~\ref{app:gap_dp} to obtain $\pi^\star(\bm{c})$.
  \item \textbf{Geometry step}: take a gradient step on $\bm{c}$ using $\partial\Phi/\partial\bm{c}$ evaluated at the policy's sensor choices---the same adjoint gradient computation used in the fixed-design optimization.
  \item Repeat until convergence.
\end{enumerate}

\paragraph{Reduction by regime.}
The ignorance gap determines how much of this machinery is actually needed:
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
...
When the Jacobian $\bm{F}$ depends only weakly on $\bm{x}$, the ignorance gap is near zero and adaptation has negligible value.  To build intuition for the opposite regime, we describe several problem classes where $IG(\bm{x})$ is large and has non-trivial structure.
...
\paragraph{Optimal policy.}
The optimal adaptive policy prescribes a two-phase strategy: (1)~\emph{support identification} using broad measurements (e.g., random projections), then (2)~\emph{targeted estimation} using measurements concentrated on the identified support.  This is the adaptive compressed sensing strategy of Haupt et al.\ (2011), recovered here from the gap DP of Appendix~\ref{app:gap_dp} as the solution that minimizes $\sum \E[\Delta_k]$.
...
\midrule
Weakly nonlinear $\bm{F}$ & $\approx 0$ & Point mass at 0 & Negligible \\
...
%===============================================================================
\section{Derivation of the Bellman Equation}
\label{app:bellman}
%===============================================================================

We derive the Bellman equation \eqref{eq:bellman} from first principles for the adaptive D-optimal sensing problem.  The argument is completely general to any finite-horizon sequential decision problem; we specialize the notation to the setting of this paper.

\subsection{Problem setup}

The ingredients are:
\begin{itemize}[nosep]
  \item An unknown state $\bm{x} \sim p_0(\bm{x})$ (the prior).
  \item A horizon of $N$ decision steps, $k = 1, 2, \ldots, N$.
  \item At step $k$, we pick an action $\bm{s}_k \in \R^{d_s}$ and observe
  \begin{equation}
    \bm{y}_k = \bm{h}(\bm{x}, \bm{s}_k, \bm{c}) + \bm{v}_k, \qquad \bm{v}_k \sim \mathcal{N}(\bm{0}, \sigma^2 \bm{I}).
  \end{equation}
  \item An adaptive policy $\pi = (\pi_1, \ldots, \pi_N)$ where $\pi_k: \bm{y}_{1:k-1} \mapsto \bm{s}_k$.
  \item A terminal reward $\Phi(\bm{x}, \bm{s}_{1:N}) = \ln\det\bm{J}_N(\bm{x}, \bm{s}_{1:N}, \bm{c})$, accrued only at the end.
\end{itemize}

Our goal is to find the policy that maximizes expected terminal reward:
\begin{equation}
  \pi^\star = \argmax_\pi \; \E_{\bm{x},\, \bm{y}_{1:N}}\!\left[\Phi(\bm{x}, \pi(\bm{y}_{1:N-1}))\right],
  \label{eq:app:target}
\end{equation}
and we denote the optimum by $V_{\text{adaptive}} = \max_\pi \E[\Phi]$.  Problem \eqref{eq:app:target} is a maximization over \emph{functions} $\pi_k$, an infinite-dimensional object---hopeless at face value.
...
\begin{equation}
  ...
  \label{eq:app:belief}
\end{equation}
Two observation histories yielding the same posterior $b_k$ are equivalent going forward.  So we can equivalently search over policies of the form $\pi_k: b_k \mapsto \bm{s}_k$---the POMDP reformulation.  Boundary cases:
\begin{itemize}[nosep]
  \item $b_1 = p_0$ (prior, no observations yet);
  \item $b_{N+1} = p(\bm{x} \mid \bm{y}_{1:N})$ (posterior after all $N$ observations).
\end{itemize}
...
\begin{equation}
  ...
  \label{eq:app:bayes}
\end{equation}
Crucially, the marginal distribution of $\bm{y}_k$ given $(b_k, \bm{s}_k)$ is
\begin{equation}
  p(\bm{y}_k \mid b_k, \bm{s}_k) \;=\; \int p(\bm{y}_k \mid \bm{x}, \bm{s}_k)\, b_k(\bm{x})\, d\bm{x}.
  \label{eq:app:marginal}
\end{equation}

\subsection{Value-to-go}

The \emph{value-to-go} $W_k(b_k)$ is the best achievable expected terminal reward, starting from belief $b_k$ at step $k$, using the optimal policy for the remaining steps $k, k+1, \ldots, N$:
\begin{equation}
  W_k(b_k) \;=\; \max_{\pi_k, \pi_{k+1}, \ldots, \pi_N}\; \E\!\left[\Phi(\bm{x}, \bm{s}_{1:N}) \mid b_k, \pi_{k:N}\right].
  \label{eq:app:vtg_def}
\end{equation}
(The already-committed prefix $\bm{s}_{1:k-1}$ is held fixed; we suppress it notationally.)  Two boundary cases:
\begin{itemize}[nosep]
  \item \textbf{$k = 1$:} $W_1(\text{prior}) = V_{\text{adaptive}}$---the quantity we actually care about.
  \item \textbf{$k = N+1$:} no decisions remain; $\bm{s}_{1:N}$ are all committed; the terminal value is simply the expected reward under the final belief,
  \begin{equation}
...
\subsection{The key manipulation: peel off one step}

This is the heart of the derivation.  Start from \eqref{eq:app:vtg_def} and split the outer max into ``choose $\bm{s}_k$ now'' and ``choose the remaining $\pi_{k+1:N}$'':
\begin{equation}
  W_k(b_k) \;=\; \max_{\bm{s}_k}\; \max_{\pi_{k+1}, \ldots, \pi_N}\; \E\!\left[\Phi \mid b_k, \bm{s}_k, \pi_{k+1:N}\right].
  \label{eq:app:split}
\end{equation}
Now apply the \emph{tower property} (law of total expectation), conditioning on the next observation $\bm{y}_k$:
\begin{align}
  \E\!\left[\Phi \mid b_k, \bm{s}_k, \pi_{k+1:N}\right] &= \E_{\bm{y}_k \mid b_k, \bm{s}_k}\!\left[\E\!\left[\Phi \mid b_{k+1}, \pi_{k+1:N}\right]\right],
  \label{eq:app:tower}
\end{align}
because $b_{k+1}$ is a deterministic function of $(b_k, \bm{s}_k, \bm{y}_k)$ via Bayes \eqref{eq:app:bayes}, and all subsequent randomness depends on $b_{k+1}$ only through the belief (Markov property of beliefs).

Substituting \eqref{eq:app:tower} into \eqref{eq:app:split} and pushing the $\max$ over $\pi_{k+1:N}$ \emph{inside} the expectation $\E_{\bm{y}_k \mid b_k, \bm{s}_k}$ (valid because $\bm{y}_k$ is drawn before $\pi_{k+1:N}$ is applied):
\begin{equation}
  W_k(b_k) \;=\; \max_{\bm{s}_k}\; \E_{\bm{y}_k \mid b_k, \bm{s}_k}\!\left[\;\max_{\pi_{k+1}, \ldots, \pi_N}\; \E\!\left[\Phi \mid b_{k+1}, \pi_{k+1:N}\right]\right].
  \label{eq:app:pushed}
\end{equation}
The inner quantity in square brackets is \textbf{exactly $W_{k+1}(b_{k+1})$} by the definition \eqref{eq:app:vtg_def} applied at step $k+1$.  Therefore:
\begin{equation}
  \boxed{\;W_k(b_k) \;=\; \max_{\bm{s}_k}\; \E_{\bm{y}_k \mid b_k, \bm{s}_k}\!\left[W_{k+1}(b_{k+1})\right].\;}
  \label{eq:app:bellman}
\end{equation}
This is the \textbf{Bellman equation}: the optimal value at $(k, b_k)$ equals the best over actions $\bm{s}_k$ of the expected optimal value at the next belief $b_{k+1}$, where the expectation is over $\bm{y}_k \sim p(\bm{y}_k \mid b_k, \bm{s}_k)$.

\subsection{Why the push-in of the max is legal}

Equation \eqref{eq:app:pushed} involved moving $\max_{\pi_{k+1:N}}$ inside $\E_{\bm{y}_k \mid b_k, \bm{s}_k}$.  In general, $\E[\max] \neq \max[\E]$ (max is convex, so $\E[\max] \ge \max[\E]$ by Jensen's inequality---this is exactly the one-step Jensen gap $\Delta_k$ of \eqref{eq:jensen_gap}).  So why is the push-in legal here?

The answer: \textbf{we are \emph{not} commuting the operations abstractly}.  We are exploiting the fact that $\pi_{k+1:N}$ can \emph{depend on $\bm{y}_k$} (through $b_{k+1}$).  A policy that observes $\bm{y}_k$ before committing $\pi_{k+1:N}$ is strictly more powerful than one that commits in advance, and the former is what our original problem \eqref{eq:app:target} allows.  Formally, for each realization of $\bm{y}_k$, we choose the optimal $\pi_{k+1:N}$ as a function of that realization---i.e., we choose a conditional plan $\pi_{k+1:N}(\bm{y}_k, \cdot)$ pointwise in $\bm{y}_k$.  Maximizing pointwise inside the expectation gives the same value as maximizing over all such conditional plans outside.  This is the \emph{interchangeability lemma} for adaptive optimization, and it is the rigorous form of Bellman's \emph{principle of optimality}: \emph{an optimal policy's continuation is optimal from whatever state results from its first decision}.

\subsection{The full Bellman system}

Combining the recursion and the boundary condition, the optimal adaptive policy is characterized by:
\begin{align}
  ...
  V_{\text{adaptive}} &\;=\; W_1(\text{prior}). \label{eq:app:sys_value}
\end{align}

\subsection{Action-value (\texorpdfstring{$Q$}{Q}) decomposition}

Define the action-value at step $k$:
\begin{equation}
  \mathcal{Q}_k(b_k, \bm{s}_k) \;=\; \E_{\bm{y}_k \mid b_k, \bm{s}_k}\!\left[W_{k+1}(b_{k+1})\right].
  \label{eq:app:Q}
\end{equation}
Then \eqref{eq:app:sys_rec} becomes
\begin{equation}
  W_k(b_k) \;=\; \max_{\bm{s}_k}\; \mathcal{Q}_k(b_k, \bm{s}_k), \qquad \pi^\star_k(b_k) \;=\; \argmax_{\bm{s}_k}\; \mathcal{Q}_k(b_k, \bm{s}_k).
\end{equation}

In the gap DP of Section~\ref{subsec:ignorance_gap}, $\mathcal{Q}_k$ is further decomposed into an \emph{exploit} part $P_k(\bm{s}_k) = \E_{\bm{x} \mid b_k}[V_{k+1}^\star(\bm{x}; \bm{s}_k)]$ minus an \emph{explore-cost} part $Q_k(\bm{s}_k) = \E_{\bm{y}_k \mid b_k, \bm{s}_k}[\Gamma_{k+1}(b_{k+1})]$, so that $W_k(b_k) = \max_{\bm{s}_k}[P_k(\bm{s}_k) - Q_k(\bm{s}_k)]$.  This is the same Bellman equation, recast to expose the exploration--exploitation tradeoff.

\subsection{Sanity check: one step back from the terminal}

At $k = N$, the Bellman recursion \eqref{eq:app:sys_rec} gives
\begin{align}
  ...
  \label{eq:app:sanity}
\end{align}
where the second line uses the tower property: $\E_{\bm{y}_N \mid b_N, \bm{s}_N} \E_{\bm{x} \mid b_{N+1}}[f(\bm{x})] = \E_{\bm{x} \mid b_N}[f(\bm{x})]$ for any $f$ not depending on $\bm{y}_N$, because $\bm{y}_N$ is marginalized out.  Equation \eqref{eq:app:sanity} is the \emph{certainty-equivalent} evaluation at the last step: with no future to plan for, the optimal action myopically maximizes expected terminal reward under the current belief.  This is consistent with the gap DP reduction at step $N$: since $\Gamma_{N+1} = 0$, the future-cost term $Q_N \equiv 0$, so $W_N = \max_{\bm{s}_N} P_N(\bm{s}_N)$---pure exploit, no explore.

\subsection{Why the equation is powerful and cursed}

\paragraph{Powerful.}  The Bellman equation reduces an intractable optimization over \emph{functions} (policies on the space of observation histories) to a recursion over \emph{single-step} decisions.  If one can evaluate the $\max$ and $\E$ at each step---e.g., by discretizing actions and observations---the problem is solved by $N$ sweeps of dynamic programming.

\paragraph{Cursed.}  The belief $b_k$ is a probability distribution over $\bm{x}$, i.e., a point in an infinite-dimensional simplex.  $W_k(\cdot)$ is a function on this simplex.  Representing $W_k$ exactly is generally impossible except in special cases (linear-Gaussian, where $b_k$ is Gaussian and $W_k$ admits a closed form via the Kalman filter; finite discrete states).  This is why the gap DP \eqref{eq:gap_dp} developed in Appendix~\ref{app:gap_dp} is generally intractable and why approximations---certainty-equivalent policies, Jensen-gap bounds---are needed in practice.
...
\end{equation}
three moves give the Bellman equation:
\begin{enumerate}[nosep,label=(\alph*)]
  \item \textbf{Sufficient statistic reduction}: replace the history $\bm{y}_{1:k-1}$ with the belief $b_k$.
  \item \textbf{Peel off one action}: split $\max_{\pi_{k:N}} = \max_{\bm{s}_k} \max_{\pi_{k+1:N}}$.
  \item \textbf{Tower property plus interchangeability}: push $\max_{\pi_{k+1:N}}$ inside $\E_{\bm{y}_k}$, yielding $W_{k+1}(b_{k+1})$ by definition.
\end{enumerate}
The result is the recursion
\begin{equation}
...
\end{equation}
The derivation rests on exactly two facts: \textbf{(1) the Markov property of beliefs under Bayesian updating}, and \textbf{(2) the principle of optimality} (the optimal continuation policy can depend on the intervening observations without loss).

%===============================================================================
\section{Adaptive Solvers via the Gap Dynamic Program}
\label{app:gap_dp}
%===============================================================================

The Bellman equation \eqref{eq:bellman} and the conditional ignorance gap $\Gamma_k(b_k)$ \eqref{eq:cond_gap} are defined in the main text.  This appendix reformulates the Bellman recursion directly on $\Gamma_k$---the \emph{gap dynamic program}---which exposes the exploration--exploitation tradeoff of optimal adaptive sensing in closed form and admits analytical simplifications in several regimes (linear measurements, fast posterior concentration, Lipschitz oracle map).  It also gives a practical two-phase policy for the moderate-$\E[IG]$ regime.  The development is problem-agnostic: the equations apply to any sequential sensing problem with a D-optimal terminal reward.

\subsection{The gap dynamic program}
...
\begin{equation}
  \boxed{\Gamma_k(b_k) = \Delta_k(b_k) + \underbrace{\max_{\bm{s}_k}P_k(\bm{s}_k) - \max_{\bm{s}_k}[P_k(\bm{s}_k) - Q_k(\bm{s}_k)]}_{\displaystyle \ge Q_k(\bm{s}_k^{\text{full}}) \ge 0}.}
  \label{eq:gap_dp}
\end{equation}
...
When the myopic and full-Bellman actions coincide ($\bm{s}_k^P = \bm{s}_k^{\text{full}}$), the exploitation regret vanishes and the recursion simplifies to $\Gamma_k = \Delta_k + Q_k(\bm{s}_k^P)$.  Dropping the nonnegative exploitation regret gives the cleaner lower bound $\Gamma_k(b_k) \ge \Delta_k(b_k) + Q_k(\bm{s}_k^{\text{full}})$, which unrolls cleanly along the optimal trajectory.

\paragraph{Boundary condition.}
At the terminal step, $\Gamma_{N+1} = 0$ (no remaining decisions).  Under the optimal policy, $\bm{s}_k^{\text{full}}$ is the committed action at step $k$, so by the tower property, $Q_k(\bm{s}_k^{\text{full}}) = \E_{\bm{y}_k|b_k,\bm{s}_k^{\text{full}}}[\Gamma_{k+1}(b_{k+1})]$.  Taking expectation over the trajectory induced by the optimal policy and telescoping gives:
\begin{equation}
...
\subsection{Interpretation: exploration--exploitation from first principles}
...
\subsection{When the gap DP simplifies}
...
\subsection{Practical policy from the gap DP}

The gap DP \eqref{eq:gap_dp} suggests a practical two-phase policy:

\begin{enumerate}[nosep]
  \item \textbf{Monitor} $\Delta_k(b_k)$ at each step.  This requires the oracle map $\bm{x} \mapsto \bm{s}^\star(\bm{x})$, which is an $N d_s$-variable optimization per $\bm{x}$, evaluated over samples from the current posterior.
