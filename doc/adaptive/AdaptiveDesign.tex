...
\paragraph{Applicability: continuous vs.\ discrete state spaces.}
The PCRB formulation above presupposes a \emph{continuous} state: the Fisher information in \eqref{eq:JP} is defined through derivatives of the log-likelihood with respect to $\bm{x}$, which requires $\bm{x}$ to be a continuous parameter for the score $\partial \ln p(\bm{y} \mid \bm{x}) / \partial \bm{x}$ to exist.  For discrete state spaces---e.g., the $K$-cell target-search setup of Section~\ref{sec:examples}, where $z \in \{1, \ldots, K\}$ is a cell index---the PCRB baseline \eqref{eq:vfixed_pcrb} \textbf{cannot be formulated at all}: Fisher information is undefined at every $\bm{x}$ because the derivative does not exist, and there is no discrete analog with a matrix-inverse structure that plays the role of $\bm{J}_P^{-1}$ as an MSE bound.  (Related discrete-parameter quantities exist---pairwise Kullback--Leibler divergences, Chernoff and Bhattacharyya information, Fano's inequality---but none of these packages into a single positive-definite matrix whose inverse bounds a covariance.  They bound classification error, not MSE.)

The $\Phi$-based formulation of Section~\ref{sec:reward}, by contrast, applies identically to discrete and continuous $\bm{x}$.  The differential entropy $H$, the KL divergence, and the mutual information used to define $\Phi$ are well-defined in both settings (integrals become sums for discrete $\bm{x}$, with no change in content), and the oracle, ignorance-gap, and Bellman-reward machinery runs verbatim.  This scope asymmetry is more than a technicality: it means the PCRB baseline \eqref{eq:vfixed_pcrb} is operationally available as an MSE-anchored reference for case studies with continuous state (e.g., the superconducting-qubit flux sensor) but \emph{not} for case studies with discrete state (e.g., the radar target-search benchmark).  The $\Phi$ framework serves both uniformly, and the comparison $\overline{\mathrm{MSE}}_2/\overline{\mathrm{MSE}}_1$ of Section~\ref{subsec:pcrb_baseline} can only be reported for the former class of problems.
...
Let
\begin{equation}
  (\bm{c}_1^\star,\, \pi_1^\star) \;=\; \argmax_{\bm{c},\,\pi}\; W_1(p_0;\, \bm{c}) \qquad\text{(joint }\bm{c}+\text{DP result of Section~\ref{sec:joint})}
\end{equation}
and
\begin{equation}
  (\bm{c}_2^\star,\, \bm{s}_{1:N}^{(2)\star}) \;=\; \argmax_{\bm{c},\,\bm{s}_{1:N}}\; \ln\det \bm{J}_P(\bm{s}_{1:N}, \bm{c}) \qquad\text{(PCRB-baseline result \eqref{eq:pcrb_baseline})}.
\end{equation}
Denote by $\overline{\mathrm{MSE}}_1$ and $\overline{\mathrm{MSE}}_2$ the respective prior-averaged mean-square errors achieved in deployment---the first under the adaptive policy $\pi_1^\star$ at geometry $\bm{c}_1^\star$, the second under the fixed schedule $\bm{s}_{1:N}^{(2)\star}$ at geometry $\bm{c}_2^\star$.  Both are concrete operational quantities measurable from Monte Carlo simulation or from deployed data.

The ratio
\begin{equation}
  \mathrm{MSE\ gain} \;=\; \frac{\overline{\mathrm{MSE}}_2}{\overline{\mathrm{MSE}}_1}
\end{equation}
gives a reviewer-friendly, estimator-level, MSE-denominated statement of how much the joint-$(\bm{c}, \pi)$ DP formulation buys over the classical PCRB-optimal fixed design.  Unlike an information-gain ratio---which invites the reviewer's ``but what does this buy me in MSE?''---the PCRB-anchored ratio answers that question directly, in the same units in which the sensor's deployed performance will be reported.

Note that $\overline{\mathrm{MSE}}_1 < \overline{\mathrm{MSE}}_2$ is \emph{not} guaranteed by theory: the PCRB baseline solves a different (prior-averaged) problem from the Bellman DP (pointwise-Bellman-expected), and the two objectives can disagree on which $\bm{c}$ is best even within the Gaussian regime where they are closest.  Empirically checking that $\overline{\mathrm{MSE}}_1 < \overline{\mathrm{MSE}}_2$ is therefore a meaningful test---not a tautology---and a passing result is a quantitative endorsement of the joint-DP framework in the currency that matters for deployment.
