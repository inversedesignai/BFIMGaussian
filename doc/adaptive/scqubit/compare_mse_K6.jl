#=
compare_mse_K6.jl — MSE comparison at K=6, J=4, L=2 (realistic box).
=#
...
joint = deserialize(joinpath(@__DIR__, "results", "joint_K6", "final.jls"))
...
println("\n[Deploy joint-DP (MSE terminal, K=6, J=4)]")
t0 = time()
...
println("\n" * "="^72)
println("HEADLINE (K=6, J=4, L=2, realistic box, MSE-terminal)")
println("-"^72)
...
open(joinpath(@__DIR__, "results", "compare_mse_K6.jls"), "w") do io
