#=
sweep_joint_K4_K256.jl — K=4, J=10, L=2, TRAINING at K_PHI=256 (fine grid).

K=3 K_PHI sweep showed finer training grid gives notably better deployed MSE
(1.89e-2 at K_PHI=256 vs 2.10e-2 at K_PHI=64).  This script retrains the K=4
joint-DP policy on the same K_PHI=256 grid used at deployment, eliminating
the grid-mismatch bias.

Memo: still ~8.6M entries (memo keyed by counts, not K_PHI).  Each DP
recursion is 4x slower in phi-grid ops → ~240s per Bellman solve.
=#
using Printf
using Serialization
using Dates
...
println("sweep_joint_K4_K256.jl — K=4, J=10, L=2, K_Φ=256 training")
...
(c_final, hist, memo_final) = joint_opt(PAPER_BASELINE;
    ...
    ckpt_every=5,
    ckpt_dir=joinpath(@__DIR__, "results", "joint_K4_K256"),
    cbox=box,
    terminal=:mse,
    grad_method=:zygote,  # Zygote is faster per-iter despite higher memory
    verbose=true)
@printf("\nTotal elapsed: %.1f min\n", (time() - t0) / 60)
flush(stdout)

out_path = joinpath(@__DIR__, "results", "joint_K4_K256", "final.jls")
