#=
sweep_pcrb_K4.jl — PCRB baseline at K=4, J=10, L=2.

Matches sweep_joint_K4.jl geometry exactly for a like-for-like comparison.
Inner schedule enumeration over (J·L)^K = 20^4 = 160k combinations per reopt.
=#
using Printf
using Serialization
using Dates
...
println("sweep_pcrb_K4.jl — K=4, J=10, L=2")
...
const N_GRID      = (1, 10)
const OUTER_ITERS = parse(Int, get(ENV, "PCRB_ITERS", "200"))
...
outdir = joinpath(@__DIR__, "results", "pcrb_K4")
...
@printf("log J_P best @ iter %d = %.4f  (init %.4f, Δ=%.4f)\n",
        imax, hist.log_JP[imax], hist.log_JP[1], hist.log_JP[imax] - hist.log_JP[1])
println("best sched idx: ", hist.sched[imax])
