#=
sweep_joint_K4.jl — longer horizon K=4, J=10, L=2.

Rationale: at K=3 (current best), both joint-DP and PCRB converge to a
long-τ schedule that suffers from Ramsey-fringe aliasing. Adaptive's win
is limited to 13.4% because only 1 of 3 epochs can be "spent" on
coarse-disambiguation. With K=4, adaptive gains a second disambiguation
opportunity and the gap should widen.

J=10 log-spaced in [10, 320] ns keeps PCRB enumeration tractable
((10*2)^4 = 160k schedules per reopt) while preserving the coarse-fine τ
ladder.
=#
using Printf
using Serialization
...
println("sweep_joint_K4.jl — K=4, J=10, L=2, MSE terminal")
...
(c_final, hist, memo_final) = joint_opt(PAPER_BASELINE;
    ...
    ckpt_every=10,
    ckpt_dir=joinpath(@__DIR__, "results", "joint_K4"),
    cbox=box,
    terminal=:mse,
    verbose=true)
@printf("\nTotal elapsed: %.1f min\n", (time() - t0) / 60)

out_path = joinpath(@__DIR__, "results", "joint_K4", "final.jls")
