#=
compare_mse_fine.jl — MSE comparison for J=20 fine τ runs.
=#
...
joint = deserialize(joinpath(@__DIR__, "results", "joint_fine", "final.jls"))
pcrb  = deserialize(joinpath(@__DIR__, "results", "pcrb_fine",  "final.jls"))
...
println("\n[Deploy joint-DP (MSE terminal, J=20)]")
t0 = time()
...
println("\n" * "="^72)
println("HEADLINE (J=20 fine, L=2, realistic box, MSE-terminal)")
println("-"^72)
...
open(joinpath(@__DIR__, "results", "compare_mse_fine.jls"), "w") do io
