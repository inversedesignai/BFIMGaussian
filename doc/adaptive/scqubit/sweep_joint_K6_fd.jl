#=
sweep_joint_K6_fd.jl — K=6, J=4, L=2 using ForwardDiff gradient (low-memory).

Zygote reverse-mode blows up memory at K≥5 (300GB+ for K=6) because the tape
tracks the full recursive tree.  ForwardDiff forward-mode keeps only dual
numbers along the traversal — memory is O(depth × chunk × K_phi), i.e. tiny.
=#
using Printf
using Serialization
using Dates
...
println("sweep_joint_K6_fd.jl — K=6, J=4, L=2, MSE terminal (ForwardDiff)")
...
(c_final, hist, memo_final) = joint_opt(PAPER_BASELINE;
    ...
    ckpt_every=10,
    ckpt_dir=joinpath(@__DIR__, "results", "joint_K6_fd"),
    cbox=box,
    terminal=:mse,
    grad_method=:forwarddiff,
    verbose=true)
@printf("\nTotal elapsed: %.1f min\n", (time() - t0) / 60)
flush(stdout)

out_path = joinpath(@__DIR__, "results", "joint_K6_fd", "final.jls")
