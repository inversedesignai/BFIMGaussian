#=
compare_mse_K4.jl — MSE comparison at K=4, J=10, L=2 (realistic box).
=#
...
joint = deserialize(joinpath(@__DIR__, "results", "joint_K4", "final.jls"))
pcrb  = deserialize(joinpath(@__DIR__, "results", "pcrb_K4",  "final.jls"))
...
@printf("c_joint: f_q=%.3f E_C=%.3f kappa=%.1e Delta=%.3f\n",
...
println("\n[Deploy joint-DP (MSE terminal, K=4, J=10)]")
t0 = time()
...
println("\n" * "="^72)
println("HEADLINE (K=4, J=10, L=2, realistic box, MSE-terminal)")
println("-"^72)
...
open(joinpath(@__DIR__, "results", "compare_mse_K4.jls"), "w") do io
