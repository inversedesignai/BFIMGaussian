#=
compare_mse_narrow.jl — MSE comparison at K=4 J=10 L=2, phi_max=0.1.

Narrow-prior regime: expected large joint-DP advantage (~8× at baseline).
=#
...
joint = deserialize(joinpath(@__DIR__, "results", "joint_narrow", "final.jls"))
...
println("\n" * "="^72)
@printf("HEADLINE (K=4, J=10, L=2, phi_max=%.3f, realistic box, MSE-terminal)\n", joint.PHI_MAX)
println("-"^72)
...
open(joinpath(@__DIR__, "results", "compare_mse_narrow.jls"), "w") do io
