#=
compare_mse_K5.jl — MSE comparison at K=5, J=6, L=2 (realistic box).
=#
...
joint = deserialize(joinpath(@__DIR__, "results", "joint_K5", "final.jls"))
pcrb  = deserialize(joinpath(@__DIR__, "results", "pcrb_K5",  "final.jls"))
...
println("\n[Deploy joint-DP (MSE terminal, K=5, J=6)]")
t0 = time()
...
println("\n" * "="^72)
println("HEADLINE (K=5, J=6, L=2, realistic box, MSE-terminal)")
println("-"^72)
...
open(joinpath(@__DIR__, "results", "compare_mse_K5.jls"), "w") do io
