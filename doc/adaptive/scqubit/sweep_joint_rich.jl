#=
sweep_joint_rich.jl — richer action grid (J=6, L=4, plan defaults).

Joint-DP MSE-terminal with τ ∈ {10, 20, 40, 80, 160, 320} ns and
n ∈ {1, 3, 10, 30}.  Geom-only c box (T, A_φ, A_Ic pinned at paper).
=#
using Printf
using Serialization
using Dates

include(joinpath(@__DIR__, "ScqubitModel.jl"))
include(joinpath(@__DIR__, "Belief.jl"))
include(joinpath(@__DIR__, "Baselines.jl"))
include(joinpath(@__DIR__, "Bellman.jl"))
include(joinpath(@__DIR__, "Gradient.jl"))
include(joinpath(@__DIR__, "JointOpt.jl"))
using .ScqubitModel, .Belief, .Bellman, .Gradient, .JointOpt

println("sweep_joint_rich.jl — J=6, L=4, geom-only, MSE terminal")
println("Threads: $(Threads.nthreads())")

const K_EPOCHS    = 3
const K_PHI       = 64
const TAU_GRID    = ntuple(k -> 10e-9 * 2.0^(k-1), 6)      # 10, 20, 40, 80, 160, 320 ns
const N_GRID      = (1, 3, 10, 30)
const OUTER_ITERS = parse(Int, get(ENV, "JOINT_ITERS", "80"))
const OUTER_LR    = parse(Float64, get(ENV, "JOINT_LR", "5e-4"))
const REOPT_EVERY = parse(Int, get(ENV, "JOINT_REOPT", "10"))

"Physically realistic box: f_q_max ∈ [3, 12] GHz, E_C/h ∈ [0.15, 0.4] GHz,
κ ∈ [0.1, 5] MHz, Δ_qr ∈ [0.8, 5] GHz.  T, A_φ, A_Ic pinned at paper baseline
(40 mK, 1e-6 Φ₀, 1e-6 I_c) which are realistic dilution-fridge / flux-noise
literature values."
function realistic_box(bl::ScqubitParams)
    lo = [ 3.0e9,   0.15e9,  0.1e6,   0.8e9,  bl.temperature,  bl.A_phi,  bl.A_Ic]
    hi = [12.0e9,   0.4e9,   5.0e6,   5.0e9,  bl.temperature,  bl.A_phi,  bl.A_Ic]
    CBox(lo, hi)
end
geom_only_box(bl) = realistic_box(bl)  # alias for backward-compat with helper

grid = make_grid(; K_phi=K_PHI, phi_max=0.49, tau_grid=TAU_GRID, n_grid=N_GRID)
box  = geom_only_box(PAPER_BASELINE)

println(@sprintf("Config: K=%d K_Φ=%d J=%d L=%d iters=%d lr=%.1e reopt=%d terminal=:mse",
                 K_EPOCHS, K_PHI, length(TAU_GRID), length(N_GRID),
                 OUTER_ITERS, OUTER_LR, REOPT_EVERY))

t0 = time()
(c_final, hist, memo_final) = joint_opt(PAPER_BASELINE;
    grid=grid, K_epochs=K_EPOCHS,
    outer_iters=OUTER_ITERS, outer_lr=OUTER_LR,
    policy_reopt_every=REOPT_EVERY,
    ckpt_every=20,
    ckpt_dir=joinpath(@__DIR__, "results", "joint_rich"),
    cbox=box,
    terminal=:mse,
    verbose=true)
@printf("\nTotal elapsed: %.1f min\n", (time() - t0) / 60)

out_path = joinpath(@__DIR__, "results", "joint_rich", "final.jls")
isdir(dirname(out_path)) || mkpath(dirname(out_path))
open(out_path, "w") do io
    serialize(io, (; c_final, v_final=c_as_vec(c_final),
                     history=hist, memo_final, K_EPOCHS, K_PHI,
                     TAU_GRID, N_GRID, timestamp=now(),
                     baseline=PAPER_BASELINE,
                     fixed_components=(:temperature, :A_phi, :A_Ic),
                     terminal=:mse))
end
println("Saved final to $out_path")

nj = length(hist.c_vec)
va = hist.V_adaptive[1:nj]
ibest = argmax(va)
@printf("-E[Var_post](c₀)      = %.4e\n", va[1])
@printf("-E[Var_post](best)    = %.4e  (iter %d, Var=%.4e)\n", va[ibest], ibest, -va[ibest])
names = (:f_q_max, :E_C_over_h, :kappa, :Delta_qr, :temperature, :A_phi, :A_Ic)
for (n, v) in zip(names, hist.c_vec[ibest])
    @printf("  %-12s = %+.4e\n", n, v)
end
