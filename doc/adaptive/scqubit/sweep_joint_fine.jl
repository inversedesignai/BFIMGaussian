#=
sweep_joint_fine.jl — fine τ discretization (J=20, L=2) as "continuous" proxy.

τ log-spaced 10..320 ns (20 points).  n_grid = (1, 10) (keep simple).  K=3.
Geom-only realistic box.  MSE terminal.
=#
using Printf
using Serialization
using Dates
...
println("sweep_joint_fine.jl — J=20 fine τ, L=2")
println("Threads: $(Threads.nthreads())")

const K_EPOCHS    = 3
const K_PHI       = 64
const J_TAU       = 20
const TAU_GRID    = ntuple(k -> 10e-9 * (32.0)^((k-1)/(J_TAU-1)), J_TAU)  # log-spaced 10..320 ns
const N_GRID      = (1, 10)
...
# One-time Bellman solve to measure memo size before any Adam
t_probe = time()
...
(c_final, hist, memo_final) = joint_opt(PAPER_BASELINE;
    ...
    ckpt_every=10,
    ckpt_dir=joinpath(@__DIR__, "results", "joint_fine"),
    cbox=box,
    terminal=:mse,
    verbose=true)
@printf("\nTotal elapsed: %.1f min\n", (time() - t0) / 60)

out_path = joinpath(@__DIR__, "results", "joint_fine", "final.jls")
