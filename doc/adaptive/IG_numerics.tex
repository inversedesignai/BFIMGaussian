...
\textbf{Necessary-but-not-sufficient claim:} $\E[IG](\bm{c})$ is an upper bound on $V_{\text{adaptive}}(\bm{c}) - V_{\text{fixed}}(\bm{c})$, but is typically a \emph{loose} bound, and choosing $\bm{c}$ to maximize $\E[IG]$ does \emph{not} maximize the realized adaptive advantage.  The two-regime comparison makes this stark:
\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
$\bm{c}$ & $\E[IG]$ & $V_{\text{adaptive}} - V_{\text{fixed}}$ & saturation \\
\midrule
narrow top-hat ($m = 1$) & $\mathbf{2.081}$ & $0.121$ & $5.8\%$ \\
wide top-hat ($m = K/2$) & $0.513$ & $\mathbf{0.289}$ & $\mathbf{56.4\%}$ \\
Gaussian $w = 0.6$       & $0.635$ & $0.108$ & $17.0\%$ \\
\bottomrule
\end{tabular}
\end{center}
The narrow design has the largest EVPI but the smallest realized gain; the wide top-hat has smaller EVPI but more than $2\times$ the realized gain; and the Gaussian $w^\star$ maximum has the highest \emph{absolute} $V_{\text{adaptive}}$ but middling saturation.  Three different $\bm{c}$ values are optimal under three different criteria, and no substitute for the joint DP will find the right $\bm{c}^\star$.
}}
\end{center}

Use EVPI as the go/no-go gate.  Use the joint DP to find the actual $(\bm{c}^\star, \pi^\star)$.
