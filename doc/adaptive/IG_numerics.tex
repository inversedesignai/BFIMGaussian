...
%====================================================================
\section{Exact Bellman DP and \texorpdfstring{$V_{\text{adaptive}}$}{V\_adaptive}}
\label{sec:dp}
%====================================================================

\subsection{Algorithm}

The terminal reward $\Phi = \ln K - H(z \mid y_{1:N})$ is a nonlinear functional of the terminal belief, so Sondik's piecewise-linear-convex representation does not apply directly.  We instead do backward induction on the belief tree reachable from the uniform prior.  The recursion is
\begin{equation}
  W_k(b) = \max_{a \in \{1, \ldots, K\}} \; \sum_{y \in \{0, 1\}} P(y \mid b, a) \cdot W_{k+1}\!\bigl(T(b, a, y)\bigr), \qquad W_{N+1}(b) = \ln K - H(b),
  \label{eq:bellman_dp}
\end{equation}
where $T(b, a, y)$ is the Bayesian update of $b$ under observation $y$ when taking action $a$.

At depth $d$ from root the recursion branches by $(K \text{ actions}) \times (2 \text{ outcomes})$, giving $(2K)^N = 32^4 \approx 10^6$ terminal evaluations in the worst case.  Each evaluation is $O(K)$ for the posterior update and entropy.  Total work: $\approx 10^7$ elementary operations per regime.  Julia runtime: $\sim 0.1$~seconds.  Memory peak: a handful of belief vectors of length $K$, so $< 1$~MB.
...
\begin{tabular}{@{}lrr@{}}
\toprule
& narrow ($m{=}1$) & wide ($m{=}K/2{=}8$) \\
\midrule
$V_{\text{oracle}}$        & 2.579634 & 1.923462 \\
$V_{\text{adaptive}}$      & 0.619198 & 1.699767 \\
$V_{\text{fixed}}$         & 0.498683 & 1.410526 \\
$\E[IG]$ (EVPI bound)      & 2.080950 & 0.512935 \\
$V_{\text{adaptive}} - V_{\text{fixed}}$ (realized gain) & 0.120515 & 0.289241 \\
EVPI saturation \%         & 5.8\%    & 56.4\%   \\
\bottomrule
\end{tabular}
\caption{Exact Bellman-DP results with $K = 16$, $N = 4$, $p_{\mathrm{D}} = 0.9$, $p_{\mathrm{FA}} = 0.05$.  All numbers in nats unless noted.  ``EVPI saturation'' is the fraction $(V_{\text{adaptive}} - V_{\text{fixed}}) / \E[IG]$, showing how much of the ignorance-gap upper bound the optimal adaptive policy actually realizes.}
\label{tab:dp_results}
\end{table}

\subsection{Optimal policy tree (first two levels)}

\paragraph{Narrow regime.}  The depth-2 head of the DP-optimal policy tree:
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
history  & action \\
\midrule
$\emptyset$ (root) & cell 1 \\
$y_1 = 0$ (miss)      & cell 1 \\
$y_1 = 1$ (detect)    & cell 16 \\
$(y_1, y_2) = (0, 0)$ & cell 1 \\
$(y_1, y_2) = (0, 1)$ & cell 16 \\
$(y_1, y_2) = (1, 0)$ & cell 16 \\
$(y_1, y_2) = (1, 1)$ & cell 16 \\
...
\end{tabular}
\end{center}
The policy probes cell 1 at the first step and, if the observation was a miss, probes it \emph{again} at step 2 (because at finite noise a single miss does not definitively rule out cell 1: the posterior on cell 1 only dropped from $1/16 = 0.0625$ to $\approx 0.007$).  After a detection it switches to a different cell.

\paragraph{Wide regime.}  The depth-2 head:
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
history & action \\
...
\end{tabular}
\end{center}
The wide-beam adaptive policy shows visible binary-search structure: a detection at cell~1 (which covers cells $\{14, 15, 16, 1, 2, 3, 4\}$) narrows the support to roughly the top half of the ring; a miss narrows to the bottom half.  Step~2 at cell~4 (which covers $\{1, \ldots, 7\}$) vs.\ cell~13 (which covers $\{10, \ldots, 16\}$) is consistent with bisecting the post-step-1 support.

\subsection{Departures from idealized predictions}

Both regimes diverge from the idealized predictions of \texttt{AdaptiveDesign.tex} \S9 in the direction expected once finite Bernoulli noise is taken into account:

\paragraph{Narrow regime predicted zero adaptive benefit; actually 0.12 nats.}
The main document's §9 claim that ``under the strictly narrow-beam restriction, $V_{\text{adaptive}} = V_{\text{fixed}}$'' is exact only in the ideal $(p_{\mathrm{D}}, p_{\mathrm{FA}}) \to (1, 0)$ limit.  At finite noise the adaptive policy does realize a small but strictly positive gain---5.8\% of the EVPI ceiling.  The mechanism is visible in the policy tree: after a detection the adaptive policy \emph{switches} to a different cell (since the on-target cell is now likely resolved), whereas a pre-committed fixed schedule would continue visiting cells determined in advance.  The benefit is modest because the fixed design with 4 distinct-cell pointings is \emph{already} almost-optimally-chosen under the uniform prior, leaving adaptation little room.

\paragraph{Wide regime predicted $\ln 2 \approx 0.69$ nats gain; actually 0.29 nats.}
The \S9 idealized prediction $V_{\text{adaptive}} - V_{\text{fixed}} = \ln(K/(2N)) = \ln 2$ is an upper bound assumed to be saturated when adaptive binary search can fully resolve.  At finite $(p_{\mathrm{D}}, p_{\mathrm{FA}})$ full resolution isn't achievable in $N = 4$ noisy binary measurements (the information bound $4 \ln 2 = 2.77$ nats is not reached by either side).  Adaptive achieves 1.70~nats, fixed 1.41, leaving a realized gap of 0.29~nats---56\% of the EVPI.  The direction matches prediction but the magnitude is reduced by noise.

\paragraph{EVPI tightness structure is actually reversed from the idealization.}
The idealized analysis suggested the wide regime had tight EVPI and the narrow regime slack EVPI.  Numerically this is qualitatively preserved (56\% vs.\ 6\% saturation), but the narrow regime is \emph{more} slack than predicted (expected 0\%, got 6\%, which is in the right direction), and the wide regime is \emph{less} tight than predicted (expected 100\%, got 56\%).  The quantitative message is that \emph{EVPI is a genuinely loose upper bound at finite noise in all regimes we checked}, and the realized adaptive benefit is always strictly less than $\E[IG]$.  This strengthens rather than weakens the case for running an exact DP when adaptation is being considered.

\subsection{Key quantitative takeaway}

At the joint $(\bm{c}, \pi)$ optimum (wide beam, adaptive policy), finite-noise numerics give
\begin{equation}
  V_{\text{adaptive}}^\star - V_{\text{fixed}}^\star = 0.289 \text{ nats}
  \;\approx\; e^{0.289/2} - 1 \approx 16\% \text{ reduction in CRB volume}.
\end{equation}
This is the \emph{actual} dividend of adopting the full adaptive framework at this problem size---measurable, positive, but about half of what the idealized closed-form suggested.  The ignorance-gap go/no-go gate ($\E[IG] > 1$ nat in the narrow regime) would have recommended proceeding with the DP; post-solve, we see the realized gain was much smaller but still present.

%====================================================================
\section{Joint geometry--policy optimization (Gaussian beam)}
\label{sec:joint}
%====================================================================

The top-hat beam of \S3 is discrete in $m$, so gradient-based outer optimization of the geometry is not directly meaningful.  We now replace the beam model with a continuously parametrized Gaussian and sweep $w$ over a grid to map the joint $(\bm{c}, \pi)$ landscape.

\subsection{Gaussian-beam likelihood}

Parametrize the geometry by a single continuous scalar $w > 0$ (beamwidth):
\begin{equation}
  G_w(\alpha) = \exp\!\bigl(-\alpha^2 / (2 w^2)\bigr),
  \qquad
  p(y_k = 1 \mid z, a; w)
  = p_{\mathrm{FA}} + (p_{\mathrm{D}} - p_{\mathrm{FA}})\, G_w\bigl(\theta_a - \varphi(z)\bigr),
\end{equation}
with $\alpha$ wrapped to $[-\pi, \pi]$.  All downstream quantities---$\Phi(x, s, w)$, $V_{\text{oracle}}(w)$, $V_{\text{fixed}}(w)$, $V_{\text{adaptive}}(w)$---are smooth in $w$.  We sweep $w$ on a grid and at each value compute all three values exactly by the same algorithms as \S5--7 (symmetry-reduced enumeration for oracle/fixed, backward induction for the DP).

\subsection{Sweep results}

The total sweep (19~grid points, $w \in [0.05, \pi]$) completed in 122~seconds on a single Julia thread.

\begin{table}[h!]
\centering
\begin{tabular}{@{}rrrrrrr@{}}
\toprule
$w$ (rad) & $V_{\text{oracle}}$ & $V_{\text{adaptive}}$ & $V_{\text{fixed}}$ & $\E[IG]$ & realized gain & sat.\ \% \\
\midrule
0.05 & 2.5796 & 0.6192 & 0.4987 & 2.0810 & 0.1205 & 5.8 \\
0.10 & 2.5795 & 0.6191 & 0.4986 & 2.0809 & 0.1206 & 5.8 \\
0.15 & 2.5666 & 0.6176 & 0.4924 & 2.0742 & 0.1252 & 6.0 \\
0.20 & 2.4792 & 0.6411 & 0.4931 & 1.9861 & 0.1480 & 7.5 \\
0.25 & 2.3035 & 0.6875 & 0.5346 & 1.7689 & 0.1529 & 8.6 \\
0.30 & 2.1210 & 0.7335 & 0.5889 & 1.5320 & 0.1445 & 9.4 \\
0.35 & 1.9500 & 0.7704 & 0.6358 & 1.3142 & 0.1347 & 10.2 \\
0.40 & 1.8023 & 0.8059 & 0.6729 & 1.1294 & 0.1330 & 11.8 \\
0.50 & 1.5714 & 0.8522 & 0.7301 & 0.8414 & 0.1222 & 14.5 \\
\textbf{0.60} & \textbf{1.3922} & \textbf{0.8647} & 0.7571 & 0.6351 & 0.1077 & 17.0 \\
0.70 & 1.2426 & 0.8532 & \textbf{0.7576} & 0.4850 & 0.0956 & 19.7 \\
0.80 & 1.1192 & 0.8264 & 0.7480 & 0.3712 & 0.0784 & 21.1 \\
1.00 & 0.9118 & 0.7470 & 0.6968 & 0.2150 & 0.0502 & 23.4 \\
1.25 & 0.7041 & 0.6305 & 0.6016 & 0.1025 & 0.0290 & 28.3 \\
1.50 & 0.5819 & 0.5159 & 0.4983 & 0.0836 & 0.0176 & 21.1 \\
1.75 & 0.5284 & 0.4146 & 0.4034 & 0.1250 & 0.0112 & 9.0 \\
2.00 & 0.4698 & 0.3310 & 0.3234 & 0.1464 & 0.0076 & 5.2 \\
2.50 & 0.3415 & 0.2119 & 0.2079 & 0.1336 & 0.0041 & 3.0 \\
$\pi$ & 0.2117 & 0.1237 & 0.1215 & 0.0902 & 0.0022 & 2.4 \\
\bottomrule
\end{tabular}
\caption{Full sweep of the Gaussian-beam geometry parameter.  Joint optima in bold: $V_{\text{adaptive}}^\star = 0.865$ nats at $w = 0.60$, $V_{\text{fixed}}^\star = 0.758$ nats at $w = 0.70$.  The rightmost column is the EVPI saturation fraction.}
\label{tab:joint_sweep}
\end{table}

\subsection{Observations}

\paragraph{Optimum beam width.}
$V_{\text{adaptive}}$ peaks at $w^\star \approx 0.60$~rad.  In HPBW units (FWHM of the Gaussian, $= 2 w \sqrt{2 \ln 2} \approx 2.355\,w$), this corresponds to $\text{HPBW}^\star \approx 1.41$~rad $\approx 3.6$~cells.  This is neither the narrow-beam limit (HPBW $\ll 2\pi/K$) nor the wide-beam limit (HPBW $\sim \pi$).  The joint optimum lies in an intermediate regime where each measurement resolves a small neighborhood of cells well enough that four measurements can collectively localize the target.

\paragraph{Why V\textsubscript{oracle} decreases monotonically in $w$.}  The oracle knows $x$ and points the beam directly at it.  Narrower beams give larger $p(y=1 \mid z = x) - p(y=1 \mid z = x')$ gap, hence more information per measurement.  No finite-noise ceiling binds: as $w \to 0$, $V_{\text{oracle}} \to V_{\text{oracle}}^{\text{narrow}} = 2.58$~nats (recovering the Section~5 value for the $m = 1$ top-hat).

\paragraph{Why V\textsubscript{adaptive} and V\textsubscript{fixed} have interior maxima.}  At very narrow $w$, per-measurement information is high but \emph{only for cells near the pointing}.  With $N = 4$ pointings across $K = 16$ cells, fixed and adaptive both miss most of the prior mass most of the time.  At very wide $w$, per-measurement information degrades and the best binary partition is weakly diagnostic.  The interior optimum $w^\star \approx 0.6$ balances the two.

\paragraph{Where the EVPI saturation is highest.}  Saturation peaks at $w \approx 1.25$ rad (28.3\%), far from the V$_{\text{adaptive}}$ optimum.  At large $w$ the EVPI itself is small (around 0.1~nats) and adaptation captures a relatively large fraction of a small gap.  At the V$_{\text{adaptive}}$ optimum ($w = 0.6$), saturation is only 17\%---adaptation captures a small fraction of a moderately-sized EVPI, but the \emph{absolute} realized gain at $w = 0.6$ (0.108 nats) is closer to the maximum of the realized-gain curve (0.153 at $w = 0.25$) than the saturation percentage alone would suggest.

\paragraph{Joint optima for adaptive vs.\ fixed differ slightly.}  $V_{\text{fixed}}$ peaks at $w = 0.70$, $V_{\text{adaptive}}$ at $w = 0.60$.  The adaptive optimum prefers a slightly narrower beam because it can exploit the higher per-measurement information by routing each measurement based on earlier outcomes; the fixed design, committed to a non-adaptive schedule, benefits from a slightly wider beam that is more forgiving of pointing choices made in advance.  The difference is small ($\Delta w = 0.1$, or about 14\% relative) but consistent.

\subsection{Envelope-theorem sanity check at the optimum}

Around $w^\star = 0.6$, nearby values give
\[
  V_{\text{adaptive}}(0.5) = 0.8522, \quad
  V_{\text{adaptive}}(0.6) = 0.8647, \quad
  V_{\text{adaptive}}(0.7) = 0.8532.
\]
Central finite difference at $w = 0.6$:
\[
  \frac{V_{\text{adaptive}}(0.7) - V_{\text{adaptive}}(0.5)}{2 \cdot 0.1}
  = \frac{0.8532 - 0.8522}{0.2} = 0.005 \text{ nats/rad}.
\]
Essentially zero, consistent with $w = 0.6$ being a (coarse-grid) interior maximum.  A finer grid or an analytic envelope-theorem gradient evaluation would refine this.

\subsection{Representative optimal sensor sequences at $w^\star$}

The symmetry-reduced fixed-design search at $w = 0.6$ returns
\[
  s^\star_{\text{fixed}} = [1, 5, 9, 13],
\]
four pointings equally spaced around the ring (quarter-rotations of each other).  Any $D_K$-orbit representative is equivalent.  This differs from the top-hat narrow-beam result $s^\star = [1, 7, 9, 13]$ in that the Gaussian-beam likelihood favors maximally-separated pointings (no two within a beamwidth of each other), whereas the top-hat was indifferent to separation as long as all four cells were distinct.

\subsection{Takeaway: EVPI diagnoses, joint DP decides}
\label{subsec:takeaway}

The sweep is the cleanest possible demonstration of \emph{why joint geometry-policy optimization is irreducible}.  The four columns of Table~\ref{tab:joint_sweep}---$V_{\text{oracle}}$, $V_{\text{adaptive}}$, $V_{\text{fixed}}$, and $\E[IG]$---behave qualitatively differently as $w$ sweeps from 0 to $\pi$:
\begin{itemize}[nosep]
\item $V_{\text{oracle}}$ is monotonically decreasing.  The oracle always benefits from a narrower beam because it knows where to point.
\item $V_{\text{adaptive}}$ has an interior maximum at $w = 0.60$ rad, value $0.865$ nats.
\item $V_{\text{fixed}}$ has an interior maximum at $w = 0.70$ rad, value $0.758$ nats.  Different from both the above.
\item $\E[IG] = V_{\text{oracle}} - V_{\text{fixed}}$ inherits the monotone decrease of $V_{\text{oracle}}$ minus a shallow $V_{\text{fixed}}$ curve: $\E[IG]$ is monotonically decreasing (on this grid).
\item The \emph{realized} adaptive advantage $V_{\text{adaptive}} - V_{\text{fixed}}$ has yet another maximum, at $w = 0.25$ rad (0.153 nats).
\end{itemize}
...
\textbf{Necessary-but-not-sufficient claim:} $\E[IG](\bm{c})$ is an upper bound on $V_{\text{adaptive}}(\bm{c}) - V_{\text{fixed}}(\bm{c})$, but is typically a \emph{loose} bound, and choosing $\bm{c}$ to maximize $\E[IG]$ does \emph{not} maximize either $V_{\text{adaptive}}$ or the realized adaptive advantage.  Restricting attention to a single beam family (here, the Gaussian family) and comparing the two extremes against the interior optimum makes this stark:
\begin{center}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
$\bm{c}$ & $\E[IG]$ & $V_{\text{adaptive}}$ & $V_{\text{fixed}}$ & realized & sat. \\
\midrule
Gaussian $w = 0.05$ (small-$w$ extreme)   & $\mathbf{2.081}$ & $0.619$          & $0.499$          & $\mathbf{0.121}$ & $5.8\%$           \\
Gaussian $w = \pi$\phantom{.05} (large-$w$ extreme) & $0.090$          & $0.124$          & $0.122$          & $0.002$          & $2.4\%$           \\
Gaussian $w = 0.60$ (in-family optimum)   & $0.635$          & $\mathbf{0.865}$ & $\mathbf{0.758}$ & $0.108$          & $\mathbf{17.0\%}$ \\
\bottomrule
\end{tabular}
\end{center}
Note the \emph{non-monotone} relationship between $\E[IG]$ and the other columns.  The small-$w$ extreme has the largest $\E[IG]$ (by a factor of $\sim\!3$ over the in-family optimum and $\sim\!23$ over the large-$w$ extreme), yet only the second-largest $V_{\text{adaptive}}$ and an EVPI saturation of only $5.8\%$.  The large-$w$ extreme has both the smallest $\E[IG]$ and the smallest $V_{\text{adaptive}}$.  The interior $w = 0.60$ \emph{dominates} both extremes on $V_{\text{adaptive}}$ (the quantity we actually want to maximize) and on $V_{\text{fixed}}$, despite having neither the largest $\E[IG]$ nor the largest realized gain.  Saturation alone is also misleading: $w = 0.60$ has the highest saturation, but $w = 0.05$ has higher realized gain ($0.121 > 0.108$) by virtue of its much larger EVPI ceiling.

Conclusion: no single column---$\E[IG]$, realized gain, or saturation---picks out the in-family optimum.  Only $V_{\text{adaptive}}$ does, and only by direct evaluation.  EVPI is a go/no-go gate; the joint DP, swept over an admissible family of $\bm{c}$, picks the optimum.
}}
\end{center}

Use EVPI as the go/no-go gate.  Use the joint DP, over the full admissible family of $\bm{c}$, to find the actual $(\bm{c}^\star, \pi^\star)$.

\paragraph{Cross-family note.}
Restricting to one family is a fair within-family test of the joint-DP claim.  Across families, the comparison is also informative but not what is being claimed here: the top-hat family achieves higher $V_{\text{adaptive}}$ than any Gaussian (see the within-top-hat-family sweep in Section~\ref{subsec:tophat_sweep} below), because its sharper in-beam vs.\ out-of-beam likelihood contrast gives it strictly higher per-measurement information.  Which family is physically realizable (Gaussian, top-hat, Dolph--Chebyshev, phased-array-synthesized, etc.) is itself part of the geometry design.

\subsection{Within-top-hat-family sweep}
\label{subsec:tophat_sweep}

For completeness, we repeat the joint sweep within the \emph{top-hat} beam family, parametrized by the integer main-lobe width $m \in \{1, 2, \ldots, K\}$ (cells per lobe).  The coverage is a contiguous arc of $m$ cells centered on the pointed cell; per-cell likelihood is $p_{\mathrm{D}}$ inside the lobe and $p_{\mathrm{FA}}$ outside.  Runtime is $\sim\!23$~s for the full $m \in \{1,\ldots,16\}$ sweep on the reference laptop (same symmetry reductions as the Gaussian sweep).

\begin{table}[h]
\centering
\caption{Within-top-hat-family joint sweep over integer $m$.  In-family optimum for $V_{\text{adaptive}}$ at $m^\star = 9$; in-family optimum for $V_{\text{fixed}}$ at $m = 7$.  Odd/even adjacent $m$ values produce identical coverage for the symmetric placements and hence identical numbers.}
\label{tab:tophat_sweep}
\small
\begin{tabular}{@{}rrrrrrr@{}}
\toprule
$m$ & $V_{\text{oracle}}$ & $V_{\text{adaptive}}$ & $V_{\text{fixed}}$ & $\E[IG]$ & realized & sat.\ \% \\
\midrule
 1 & $2.580$ & $0.619$          & $0.499$          & $\mathbf{2.081}$ & $0.121$ & $5.8\%$           \\
 2 & $2.580$ & $0.619$          & $0.499$          & $2.081$          & $0.121$ & $5.8\%$           \\
 3 & $2.293$ & $1.401$          & $1.103$          & $1.190$          & $0.299$ & $25.1\%$          \\
 4 & $2.293$ & $1.401$          & $1.103$          & $1.190$          & $0.299$ & $25.1\%$          \\
 5 & $2.081$ & $1.631$          & $1.343$          & $0.737$          & $0.287$ & $38.9\%$          \\
 6 & $2.081$ & $1.631$          & $1.343$          & $0.737$          & $0.287$ & $38.9\%$          \\
 7 & $1.924$ & $1.700$          & $\mathbf{1.411}$ & $0.513$          & $0.289$ & $56.4\%$          \\
 8 & $1.924$ & $1.700$          & $\mathbf{1.411}$ & $0.513$          & $0.289$ & $56.4\%$          \\
 9 & $1.879$ & $\mathbf{1.704}$ & $1.401$          & $0.478$          & $0.303$ & $\mathbf{63.5\%}$ \\
10 & $1.879$ & $\mathbf{1.704}$ & $1.401$          & $0.478$          & $0.303$ & $\mathbf{63.5\%}$ \\
11 & $2.074$ & $1.632$          & $1.320$          & $0.754$          & $0.312$ & $41.4\%$          \\
12 & $2.074$ & $1.632$          & $1.320$          & $0.754$          & $0.312$ & $41.4\%$          \\
13 & $2.312$ & $1.389$          & $1.041$          & $1.271$          & $\mathbf{0.347}$ & $27.3\%$ \\
14 & $2.312$ & $1.389$          & $1.041$          & $1.271$          & $\mathbf{0.347}$ & $27.3\%$ \\
...
The same qualitative story as the Gaussian family holds, now with \emph{four different} interior optima across four different columns: $V_{\text{adaptive}}$ peaks at $m^\star = 9$, $V_{\text{fixed}}$ peaks at $m = 7$, realized gain peaks at $m = 13$, and $\E[IG]$ is monotonically U-shaped (minimized at the middle and largest at the extremes $m = 1$ and $m = 16$, i.e., point-like and fully-isotropic lobes).

\begin{center}
\fbox{\parbox{0.93\textwidth}{
\textbf{Within-top-hat-family three-way comparison:}
\begin{center}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
$\bm{c}$ & $\E[IG]$ & $V_{\text{adaptive}}$ & $V_{\text{fixed}}$ & realized & sat. \\
...
\end{tabular}
\end{center}
The in-family optimum $m^\star = 9$ dominates both extremes on \emph{every} actionable column---$V_{\text{adaptive}}$, $V_{\text{fixed}}$, realized gain, and saturation.  The only column on which it is \emph{not} best is $\E[IG]$, where both extremes beat it by a factor of four.  An EVPI-only geometry selection would reject $m^\star = 9$ in favor of either $m = 1$ or $m = 16$, both of which are almost three times worse on $V_{\text{adaptive}}$.
}}
\end{center}

The top-hat-family $V_{\text{adaptive}}^\star = 1.704$ nats slightly exceeds the Gaussian-family $V_{\text{adaptive}}^\star = 0.865$ nats, confirming (numerically) that the top-hat family is Pareto-superior at this $K, N, p_{\mathrm{D}}, p_{\mathrm{FA}}$ for the in-family optimum.  In an actual design setting, the realizable family is constrained by the antenna hardware; the point of the numerics is that \emph{given} a family, the joint DP recovers its optimum and EVPI does not.
