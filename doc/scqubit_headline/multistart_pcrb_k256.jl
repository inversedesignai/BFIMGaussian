#=
multistart_pcrb_k256.jl — PCRB multistart at K_PHI=256 throughout. Uses
the existing `pcrb_baseline` (Adam on c with closed-form ForwardDiff
gradient, schedule re-enumerated periodically). MMA isn't needed for
PCRB because the Fisher-info objective `log J_P(c, schedule)` is much
smoother than V_adaptive(c) and is essentially init-insensitive in
practice (deploy MSE varies <2% across inits at K_PHI=128).

Inits: paper, naive (the two most likely to find different basins).
Output: results/pcrb_mma_k256/<INIT_ID>.jls per restart.
Wall-clock per restart: ~60-90 min. PCRB enumeration is single-threaded
so 4-8 julia threads suffice (the rest don't help).
=#
...
println("multistart_pcrb_k256.jl"); flush(stdout)
...
const INIT_LIST = let raw = get(ENV, "INITS", "paper,naive")
    isempty(raw) ? String[] : split(raw, ",")
end

println("Inits: ", INIT_LIST)
println("Config: K_PHI=$K_PHI  iters=$OUTER_ITERS  lr=$OUTER_LR  reopt=$REOPT_EVERY")
flush(stdout)
...
outdir = joinpath(@__DIR__, "results", "pcrb_mma_k256")
...
for init_id in INIT_LIST
    ...
    println("\n" * "="^72)
    println("RESTART: $init_id  (PCRB at K_PHI=$K_PHI)")
    println("="^72)
...
println("\n" * "="^72)
println("PCRB K_PHI=$K_PHI MULTISTART SUMMARY")
println("="^72)
...
g = all_results[i_g]
@printf("GLOBAL BEST (PCRB K_PHI=%d): init=%s  log_JP=%.6f  c=(%.4f, %.4f, %.4f MHz, %.4f)\n",
        K_PHI, g.init_id, g.logJP_best,
...
open(joinpath(outdir, "_summary.jls"), "w") do io
    serialize(io, (; all_results, i_global=i_g, K_PHI, timestamp=now()))
end
